MODULE MOD_GetRef
    ...
    SUBROUTINE GetRef(iter)
        ...
        if (mesh_type == 'landmesh') then
            ...
            print*, "开始读取海陆非结构网格数据 in the GetRef.F90"
        end if
        ...
        if (mesh_type == 'landmesh') then
            ...
            print*, "大气非结构网格数据读取完成 in the GetRef.F90"
            ...
            print*, "海陆非结构网格数据读取完成 in the GetRef.F90 in Line"
            ...
            if (allocated(ref_th_Atmos)) ref_colnum = ref_colnum + size(ref_th_Atmos, 2)
            ...
            if (allocated(ref_th_Atmos)) then
                ref_th(:, ref_colnum:size(ref_th_Atmos, 2)+ref_colnum-1) = ref_th_Atmos
                deallocate(ref_th_Atmos)
            end if
        end if
    ...
    SUBROUTINE GetRef_Atmos(sjx_points, Atmos_id, Atmos_ii)
        ...
        print*, "Atmos_threshold num : ", ref_colnum
        ...
        if (any(refine_onelayer_Atmos .eqv. .true.)) then
            print*, "threshold_Calculation for refine_onelayer_Atmos"
            allocate(p_num(sjx_points))
            ...
            deallocate(var2d_temp)
            print*, "threshold_Calculation for refine_onelayer_Atmos: done"
        end if
    ...
    SUBROUTINE GetRef_LOC(sjx_points, LOC_id, LOC_ii)
        ...
        ref_colnum = ref_colnum + count(refine_onelayer_atmos)
        if (ref_colnum == 0) then
            print*, "no atmos threshold for GetRef in the ", mesh_type
            return
        else
            print*, "atmos_threshold num : ", ref_colnum
