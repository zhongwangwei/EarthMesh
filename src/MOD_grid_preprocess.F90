Module MOD_grid_preprocess
    ...
    SUBROUTINE Springjustment_global(num_sjx, num_dbx, num_edge, mp, wp, ngrmw, ngrwm, n_ngrwm)
        ...
        USE MOD_file_preprocess, only : distsOnEdge_save
        IMPLICIT NONE
        ...
        real(r8), allocatable :: distsOnEdge(:)
        ...
        deallocate(xem8, yem8, zem8, xew8, yew8, zew8, vp)
    ...
    SUBROUTINE set_distsOnEdge_global(num_sjx, num_dbx, num_edge, ngrwm, n_ngrwm, CellsOnEdge, edgesOnVertex, mp, distsOnEdge)
        USE consts_coms, only : r8, pi2_r8, erad8, nxp, step, beta
        ...
        allocate(mrl_bk(num_sjx)) ! 标记是n级细化还是n-1级细化
        do i = 1, max_iter, 1
            if (i > step) cycle
            if (exit_loop_step(i) == .TRUE.) cycle ! 跳过没有细化三角形的情况
            ...
            CALL dist_layers_make(dist_len, dist00, dist_layers)

            ! 标记细化三角形
            CALL refine_sjx_regional_make(i, num_sjx, mp, mrl_bk)

            ! 设置细化三角形区域的过渡区域distsOnEdge
            CALL distsOnEdge_layers_make(i, num_rc, dist_len, num_sjx, num_dbx, num_edge, ngrwm, n_ngrwm, CellsOnEdge, edgesOnVertex, dist_layers, mrl_bk, distsOnEdge)
    ...
    SUBROUTINE dist_layers_make(dist_len, dist_select, dist_layers)
        ...
        if (trim(set_dis_type) == 'linear') then
            ...
            a = mindist_select
            b = log(2.0_r8) / log(real(2*dist_len+1, r8))
            do i = 1, 2*dist_len, 1
                dist_layers(i) = a * (i+1) ** b
            end do
    ...
    SUBROUTINE refine_sjx_regional_make(iter, num_sjx, mp, mrl_bk)
        USE NETCDF
        ...
        real(r8), allocatable, intent(in) :: mp(:,:)
        integer,  allocatable, intent(inout) :: mrl_bk(:)
        ...
        ! 根据IsInPaArea_grid对mrl_bk进行赋值，判断是否为细化边界内的三角形
        ...
        do i = num_mp_step(iter), num_sjx, 1
            ...
            if (IsInPaArea_grid(minlon_source, maxlat_source) == 1) mrl_bk(i) = 1 !标记在细化区域的三角形
        end do
    ...
    SUBROUTINE distsOnEdge_layers_make(iter, num_rc, dist_len, num_sjx, num_dbx, num_edge, ngrwm, n_ngrwm, CellsOnEdge, edgesOnVertex, dist_layers, mrl_bk, distsOnEdge)
        ...
        integer, allocatable, intent(inout) :: mrl_bk(:)
        real(r8), intent(inout) :: distsOnEdge(num_edge)
        integer, allocatable :: mrl_in(:), isbdy_array(:), IsEdgesMove(:)
    ...
    SUBROUTINE spring_dynamics_global(ngr, num_dbx, num_edge, nEdgesOnCell, edgesOnCell, CellsOnEdge, distsOnEdge, EdgesOnedge_tri, xew, yew, zew)
        ...
        do iter = 1, niter_refine
            ...
            do iu = 2, num_edge
                ...
                distm = distsOnEdge(iu) / 1.2 * ratio
    ...
    real(r8) function arc_length(ax, ay, az, bx, by, bz)
        ...
        arc_length = r * 2.0_r8 * asin(c/(2.0_r8*r))
    ...
    SUBROUTINE GetArea(num_sjx, num_dbx, num_edge, xem, yem, zem, xew, yew, zew, xev, yev, zev, cellsOnVertex, edgesOnVertex, cellsOnEdge, &
        ...
        integer :: i, j, k, num_edges, cell1, edge1
        real(r8) :: a, b, c, s, num(3), tanqe, x(3), y(3), z(3), temp, temp_relative
        real(r8) :: error_max, error_avg

        do i = 2, num_sjx, 1
            x(1) = xem(i); y(1) = yem(i); z(1) = zem(i) ! 获取三角形中心点坐标
            do j = 1, 3, 1
                cell1 = cellsOnVertex(j, i)
                x(2) = xew(cell1); y(2) = yew(cell1); z(2) = zew(cell1) ! 获取三角形顶点坐标
                do k = 1, 3, 1
                    edge1 = edgesOnVertex(k, i) ! 判断边界是否合适
                    if ((cell1 /= CellsOnEdge(1, edge1)) .and. &
                        (cell1 /= CellsOnEdge(2, edge1))) cycle           
                    x(3) = xev(edge1); y(3) = yev(edge1); z(3) = zev(edge1) ! edge
                    kiteAreasOnVertex(j, i) = kiteAreasOnVertex(j, i) + triangle_signed_area_sphere(x, y, z)
                end do
            end do
        end do
    ...
    SUBROUTINE Get_Edge_DIS_Angle(num_edge, cellsOnEdge, verticesOnEdge, &
        ...
        do iv = 2, num_edge, 1
            ...
            sign = planeAngle(xe, ye, ze, xn, yn, zn, xv, yv, zv)
            if (sign >= 0.0_r8) then
                angle = angle
            else
                angle =-angle
    ...
    real(r8) function planeAngle(ax, ay, az, bx, by, bz, cx, cy, cz)
        ...
        Dy = -((ABx * ACz) - (ABz * ACx))
        Dz =   (ABx * ACy) - (ABy * ACx)
        if ((Dx*ax + Dy*ay + Dz*az) >= 0.0_r8) then
    ...
    SUBROUTINE set_weightsOnEdge(num_sjx, num_dbx, num_edge, maxEdges, maxEdges2, vertexDegree, &
        ...
        real(r8), parameter :: uZonal = 1.0, uMeridional = 0.0
        ! 自由大气中环流主要是纬向zonal(东西方向)，经向meridional(南北方向)很弱
        allocate(error_segment(num_edge)); error_segment = 0.
        do iEdge = 2, num_edge, 1
