...
## 32. 四个绕行开关叠加后仍停在同一处（2026-07-31）

### 32.1 开关清单

本轮为绕过 Case 9 的连续失败，累计实现四个默认关闭的开关：

| 开关 | 作用 | 实测 |
|---|---|---|
| `EARTHMESH_M0_CROSS_LEVEL_SUPPORT` | 跨层父支撑请求与回滚重建 | pass 2 收敛 `39 -> 5 -> 0` |
| `EARTHMESH_M0_COMPONENT_TRIPLET_DROP` | 按分量判三元组，退让不合格分量 | 5 分量退 1，`82/1181` 面 |
| `EARTHMESH_M0_COVERAGE_RELAXATION` | 覆盖校验记账而非否决 | `conceded = 0`，从未触发 |
| `EARTHMESH_M0_SUPPORT_ORACLE_BEST_EFFORT` | repair 失败时仍从当前周界提取支撑 | 从未触发（预言器未失败） |

外加一次把退让缺口折回支撑循环的改动（`take_post_drop_support_lineages`）。

**四个开关全开、加折回改动之后，运行仍以逐字相同的失败退出**：

`Current nested grid 3 crosses the parent boundary in Method-C transition at
W face 517469 (mrlw=1, lon=-71.974, lat=-52.473)`

折回未生效，因为退让发生在 pass 2 的最后一次尝试中——此时支撑循环已跑完三轮退出，
`take_post_drop_support_lineages()` 不再有调用点。

### 32.2 一处计数口径澄清

§31.3 记录 `unsupported_witness_faces=87`，本轮改为按 lineage 去重后为 `76`。二者测量
不同对象——前者是**面数**（`.count()`），后者是**去重 lineage 数**，多个面可共享一个
lineage。**不是非确定性。**
...
1. **Method-C 能合法化 93% 的细化**——退让仅需 1/5 分量、`82/1181` 面；
2. **退让的代价会外溢**——退 `82` 面制造 `76` 个 lineage（`87` 个面）的新支撑缺口，
   外溢量大于退让量；
3. **四个约束逐个绕过后，失败点不移动**——每绕开一个约束就撞上下一个，且最终面不变。

### 32.4 判断：停止继续加开关

这与 §14–§22 经穷举搜索空间得到的结论一致：**现有 canonical 模板族下 Case 9 大概率无解。**
两条独立路径——一条穷举变量域，一条逐个绕过约束——指向同一处。

继续加第五、第六个开关的预期收益为负：每个都会在自己的位置正确生效，然后停在同一个面上。

### 32.5 三个候选方向

| | 方向 | 依据 |
|---|---|---|
| 1 | **把结论固化为不可满足性证书** | §1.3 早已写明的产品级出路；现有证据足以支撑「在当前模板族、`max_level=3`、NXP=243 下不可满足」并给出冲突核心 |
| 2 | **形态二**：Method-C 出基底 + gridfile 之后二分 | §24 两端均已实测（`840,025` 单元全门 `pass`；分级细化价数 `<= 6`、拓扑硬门全过）。代价是 tri 产品线放弃 Method-C 变分辨率 |
| 3 | 清理这四个开关 | 目前是默认关闭的诊断，证据价值已记录在案 |

四个开关均保持默认关闭；生产语义未变。Case 9 状态仍为 `INCOMPLETE`。


## 33. 形态二的验证缺口：真实需求没有产物留存（2026-07-31）

### 33.1 尝试

§24 验证了形态二的两端（`max_level=1` 基底 `840,025` 单元全门 `pass`；分级细化价数
`<= 6`、拓扑硬门全过），但**未验证中间那步**：二分能否真的满足 15″ hard demand。

拟做的实验是：取合法基底，按真实需求做需求驱动的分级细化，用 §26 的换算填
`refine_level`，再跑覆盖门。
...
| 产物来源 | 是否含真实需求 |
|---|---|
| `max_level=1` 成功运行的 `*.source-demand.json` | ❌ `base_hard_levels` / `hard_levels` / `regularized_levels` **全部 `259200` 项为零** |
| `max_level=3` 失败运行 | ❌ 不产出 demand artifact（`0` 个） |
| 合法化 checkpoint | ❌ 只含 `prepared_selected_faces`、周界、见证、patch，无需求场 |

`max_level=1` artifact 的 `max_level` 字段即为 `1`，层级已按该上限截断，**无法反推
level-3 目标**；而其硬需求本身为空，连需求**位置**都取不到。

这从数据侧再次确认 §24 的更正：那次 `pass` 是「无需求可覆盖」，不是「15″ 需求已满足」。
`product_support` 有 `179,888` 个非零 bin，但那是产品支撑范围，不是硬需求。

### 33.3 后果

**形态二无法在当前产物上验证。** 要继续需先解决需求可得性，两条路：

1. **让失败的运行也写出 demand artifact**——需求在 materialize 之前就已算出，其产出不应
   依赖网格是否成功；这是一处合理的可观测性改进；
2. **在 `max_level=3` 下单独运行需求计算**，不进入 Method-C 生成。

两者都未实现。在此之前，形态二的中间步骤（需求驱动二分 + 覆盖验收）**状态为
`unverified`**，§24.4 所列的四项待验证仍全部成立。

### 33.4 已确立的部分不受影响

- 输入端：Method-C 能产出合法基底（`840,025` 单元、全门 `pass`、`81 s`）；
- 输出端：分级细化最大价数 `6`，`dimc` 动态增长，拓扑硬门接受价数 `12`；
- 分界面在 gridfile，不在 emitter。

缺的只是把两端接起来的那一步的证据。


## 34. 真实需求只有 48 个面（2026-07-31）

### 34.1 §33 的阻塞诊断错了两层

§33 判定「形态二无法验证」的原因是「失败运行不写 demand artifact」。实际上：

1. artifact **可以**写——新增 `persist_to_path`（与 gridfile 解耦，`gridfile_hash` 留空）
   后，`max_level=3` 的失败运行确实产出了 `13 MB` 的 artifact；
2. 但其 `hard_levels` 仍**全部 `259200` 项为零**——因为 **Case 9 的需求根本不经过
   HField**。

`native_landcover_face_demands`（`native_landcover_refine.rs:20`）直接从
`FrozenLandtypeSampler` **逐面判定**并返回 `Vec<bool>`，`720x360` 的 HField 栅格在这条
路径上从不被填充。`hard_levels` 全零是正确行为，不是缺陷。

这同时追加解释了 §33 对成功运行的观察：**两条路径的需求都不在 HField 里**。

### 34.2 改为导出逐面需求

`EARTHMESH_M0_FACE_DEMAND_DUMP_DIR` 在每个 pass 的 `face_demand` 计算之后、materialize
之前落盘该 pass 的 `demanded_faces`。挂在 HField 上的那段导出已撤除——它对该路径无效，
留着会误导。

### 34.3 结果：需求比一直假设的小两个数量级

| pass | 需求面 | 总面 | 占比 |
|---:|---:|---:|---:|
| 1 | **40** | `1,180,982` | `0.0034%` |
| 2 | **8** | `1,184,168` | `0.0007%` |

**整个 Case 9 的硬需求是 `48` 个面。**

此前全部讨论使用的都是**选中面**而非需求面：

| 量 | 数值 | 含义 |
|---|---:|---|
| 真实需求 | `40` / `8` | landcover 判定要细化的面 |
| pass 2 选中面 | `1,181` | 需求 + `rad3` 足迹 + 凹角闭合 + 相位支撑 |
| 退让面 | `82` | 不合格分量 |
| 跨层支撑请求 | `39 -> 5` | 父层面 |

**`8` 个需求面撑出 `1,181` 个选中面——放大 `148` 倍。** 该放大全部来自 Method-C 的结构性
开销（足迹、闭包、过渡带），与需求量无关。

需求面在全球零散分布（`lat -67.8` 到 `77.4` 度），`38/40` 可在基底网格定位，是孤立点而非
连续区域——这与 §"hard-demand 分量形态"测得的 `90` 个连通分量、`78` 个单点一致，
且给出了更前端的来源。

### 34.4 对形态二的影响

需求 `48` 面，每面细到 level 3 需 `4` 次二分（`16` 倍单元），总增量约
`48 x 15 = 720` 个三角形——相对 `840,025` 的基底**不到 `0.1%`**。

**形态二在工作量上不构成问题。** 真正的困难始终是 Method-C 的结构放大：`48` 个需求面
导致 `1,181` 个选中面，进而导致那条长度 `22`、余数 `1` 的不合格周界。

### 34.5 状态

需求现已可得且小到可逐个检视，§33 的阻塞解除。形态二的中间步骤（需求驱动二分 +
覆盖验收）仍为 `unverified`，但已具备验证条件。Case 9 状态仍为 `INCOMPLETE`。


## 35. 一个孤立需求面就足以失败（2026-07-31）

### 35.1 起点：需求点的空间分布

§34 拿到的 `48` 个需求面带上中心坐标后，按 `500 km` 聚类得 `13` 簇：

| 簇 | 点数 | 区域 |
|---|---:|---|
| 1 | **12** | 智利南部峡湾（`-73.6~-69.5`, `-55.2~-51.5`） |
| 2 | 7 | 智利中南部（`-74.6~-72.6`, `-49.4~-45.9`） |
| 3 | 6 | 厄瓜多尔近赤道（`-78.4~-77.2`, `-2.0~2.4`） |
| 4 | 5 | 爱琴海（`15.9~24.6`, `38.4~41.0`） |
| 其余 9 簇 | 1–2 | 零散 |

失败面 `W face 517469 (-71.974, -52.473)` 落在最大簇中心，最近需求点距其 `138 km`；
pass 2 的 `8` 个需求点中有 `5` 个在其 `645 km` 内。

据此提出假设：**不可满足性由智利峡湾的破碎海岸单独造成。**

### 35.2 区域对照

以 `!Regional / !Bbox` 限定 domain，同一 NXP=243、`max_level=3` 配置：

| 实验 | bbox | pass1 需求 | pass2 需求 | 结果 |
|---|---|---:|---:|---|
| 全球 | — | `40` | `8` | `W face 517469` 跨父边界 |
| `chile-only` | `-80~-65`, `-60~-40` | `19` | `5` | `W face 509979` 跨父边界 |
| `nochile` | `-20~160`, `-80~85` | `12` | **`1`** | **`perimeter lengths [40]` non-triplet** |

需求计数与按坐标的手工聚类一致（智利占 `19/40` 与 `5/8`），确认坐标导出正确。

### 35.3 假设被否证，且否证方式更有信息量

**排除智利后仍然失败**，而此时 pass 2 只剩 **`1` 个需求面**——它单独产生了一条长度
`40` 的周界，`40 mod 3 = 1`，不可三元组分解。

> **一个孤立需求面 -> 长度 `40` 的周界 -> 余数 `1` -> 失败。**

失败不需要密集簇。智利那 `19` 个点只是让问题更大，**不是成因**。

| 假设 | 状态 |
|---|---|
| 智利峡湾破碎海岸导致不可满足 | **否证**：排除后仍失败 |
| 需求密集导致 `rad3` 足迹互相干涉 | **否证**：`1` 个需求面亦失败 |
| `rad3` 放大后周界余数不可控 | 与实测一致，且与需求量无关 |

### 35.4 与 §34 的放大量对上

- `8` 个需求面 -> `1,181` 个选中面（`148x`）；
- **`1` 个需求面 -> 周界长 `40`**。

放大是结构性的：**每个孤立需求点都独立产生一条周界，其长度由该点周围的几何决定，
没有可调自由度。** `40 mod 3 = 1` 不是选择的结果。

这比此前所有结论更接近根因：**Method-C 无法保证单个孤立需求点的过渡周界可三元组分解。**

### 35.5 可进一步验证的最小反例

上述可用人工需求直接检验：在均匀基底上指定**单个**面为硬需求，扫描其位置，统计所得
周界长度的模 3 分布。

- 若余数在 `{0,1,2}` 上大致均匀 -> **约 `2/3` 的孤立需求点天然不可满足**，这是模板族的
  固有性质而非本例特殊；
- 若余数恒为 `0` 而本例为特例 -> 需回到位置相关的解释。

该实验尚未进行。Case 9 状态仍为 `INCOMPLETE`。


## 36. 根因：海岸边界的不规则拓扑（2026-08-01）

> **[更正 2026-08-01，见 §37]** 本节结论**错误**。所有测量都做在
> `result/gridfile_NXP0243_tri.nc4` 上，而该文件是**输出阶段裁剪之后**的产物
> （`refine_pipeline/outputs.rs:473`）。细化实际发生在裁剪**之前**的
> `tmpfile/*_refine_raw_tri.nc4` 上，那是一张**闭合球面**：边界边 `3`、Euler `2`、
> 价数 `{5: 137, 6: 591821, 7: 126}`。因此「ocean mask 把规则拓扑打成 19,005 个边界点，
> rad3 足迹在那里失控」的机制不成立——细化时根本没有那些边界点。真实机制见 §37。

### 36.1 单种子周界扫描

§35.5 提出的最小反例已实现（`tests/method_c_single_seed_perimeter.rs`）：在未裁剪的
均匀基底上，对每个价数为 `6` 的内部 M 点取其 `rad3` 足迹、闭合凹角、量周界长度模 3。

| nxp | 采样 | 余 0 | 余 1 | 余 2 | 周界长度取值 |
|---:|---:|---:|---:|---:|---|
| 6 | 350 | 170 | `60` | `120` | `{16, 17, 18}` |
| 7 | 480 | 300 | `60` | `120` | `{16, 17, 18}` |
| 9 | 800 | **620（77.5%）** | `60` | `120` | `{16, 17, 18}` |

两点：**余数非零的绝对数量恒为 `60` 与 `120`，不随网格增大**；周界长度只有三个值，
`18 mod 3 = 0` 合格，`16`、`17` 不合格。

`60 = 12 x 5`、`120 = 12 x 10` 指向二十面体的十二个五边形奇点，据此提出假设：
**不合格种子集中在五边形奇点附近。**

### 36.2 假设否证：失败面离奇点 4,146 km

在真实 NXP=243 基底网格上定位价数为 `5` 的 W 点，并用「只属于一个三角形的边」区分
海岸边界点与内部奇点：
...
网格价数分布为 `{1: 3372, 2: 3940, 3: 3822, 4: 3548, 5: 4337, 6: 410228, 7: 16}`。

失败面 `W face 517469 (-71.974, -52.473)` 的距离：

| 到什么 | 距离 |
|---|---:|
| 最近的**内部奇点** | **`4,146 km`**（约 `260` 个单元） |
| 最近的**海岸边界点** | **`9.9 km`**（不到 `1` 个单元） |

**失败面紧贴海岸边界，与二十面体奇点无关。**

### 36.3 修正后的根因

`ocean mask` 裁剪把规则的价数-`6` 拓扑打成 `19,005` 个不规则边界点，其中 `4,318` 个价数
为 `5`——**不规则点由 `12` 个变成四千余个，来源是海岸线而非二十面体**。`rad3` 足迹在这些
点附近的周界长度不受控。

这回头解释 §34–§35：那 `48` 个需求面全部由 landcover 的海陆判据产生，**按定义都在海岸线
上**，因而每一个都落在不规则边界附近。

### 36.4 单种子扫描的正确读法

扫描显示规则区 `77.5%` 以上的种子给出余 `0`。但**Case 9 的需求点没有一个在规则区**。

因此正确结论不是「多数孤立点可行」，而是：

> **规则区可行，边界区不可行，而所有 landcover 需求按定义都在边界区。**

### 36.5 已否证假设汇总

| 假设 | 状态 |
|---|---|
| 地理区域（智利峡湾）导致 | 否证（§35：排除后仍失败） |
| 需求密集导致 `rad3` 足迹干涉 | 否证（§35：`1` 个需求面亦失败） |
| 约 `2/3` 孤立点天然不可满足 | 否证（本节：nxp=9 时 `77.5%` 合格） |
| 二十面体五边形奇点导致 | 否证（本节：失败面距奇点 `4,146 km`） |
| **海岸边界的不规则拓扑导致** | **与全部实测一致** |

Case 9 状态仍为 `INCOMPLETE`；本节为只读测量，未改动生产路径。


## 37. 细化在闭合球面上进行；失败面紧邻二十面体不规则点（2026-08-01）

### 37.1 顺序澄清：已经是「先细化、后判海陆」

Ocean mask 由 `write_landtype_masked_gridfile_with_hard_demand_report`
（`refine_pipeline/outputs.rs:473`）在**输出阶段**施加。细化阶段的
`native_product_support` 在原生 landcover 路径下是**全 `true`**
（`global_source.rs:742`），即细化时不做任何裁剪。

对同一次运行的两份产物实测：

| | `tmpfile/*_refine_raw_tri.nc4`（裁剪前） | `result/*_tri.nc4`（裁剪后） |
|---|---:|---:|
| 三角形 | `1,184,165` | `840,025` |
| 边界边 | **`3`** | `19,005` |
| Euler | **`2`（闭合球面）** | `-250` |
| 价数分布 | `{5: 137, 6: 591821, 7: 126}` | `{1:3372, 2:3940, 3:3822, 4:3548, 5:4337, 6:410228, 7:16}` |

**细化时的网格是闭合球面，价数几乎全为 `6`。** §36 把裁剪后产物的边界当成了细化时的
边界，全部距离测量因此失效。

顺带回答「可否先细化再判海陆」：**现状就是如此**，无需重排。

### 37.2 重测：失败面紧邻不规则点

在裁剪前网格上，不规则点（价数 `!= 6`）共 `263` 个：`137` 个价数 `5`、`126` 个价数 `7`。

失败面 `W face 517469 (-71.974, -52.473)` 到最近者：

| 距离 | 点 | 价数 |
|---:|---|---:|
| **`38.9 km`** | `W517890 (-72.00, -52.12)` | `5` |
| `46.4 km` | `W517416` | `7` |
| `63.7 km` | `W517409` | `7` |
| `71.8 km` | `W517395` | `5` |
| `94.2 km` | `W517896` | `5` |

单元尺度约 `16 km`，最近者相当于 **`2.4` 个单元**；五个最近的都在 `6` 个单元内。

### 37.3 显著性检验

`263` 个不规则点散布在 `592,084` 个 W 点中。以 `4,000` 个球面均匀随机点为基线：

| 量 | 值 |
|---|---:|
| 到最近不规则点的中位距离 | `2,101 km` |
| 5 分位 | `552 km` |
| 距离 `< 38.9 km` 的比例 | **`5/4000 = 0.12%`** |

**失败面落在不规则点 `38.9 km` 内属于 `0.12%` 的事件，不是巧合。**

### 37.4 修正后的根因

§36.1 的单种子扫描本就指向这里：余数非零的种子数恒为 `60` 与 `120`
（`= 12x5` 与 `12x10`），与网格大小无关，且长度只取 `{16, 17, 18}`。

裁剪前网格的不规则点为 `263` 个而非 `12` 个——多出的来自嵌套细化本身在过渡带引入的
价数偏差（价数 `7` 有 `126` 个，规则的二十面体不产生价数 `7`）。

因此：**`rad3` 足迹落在二十面体奇点及嵌套过渡带引入的不规则点附近时，周界长度取 `16`
或 `17` 而非 `18`，不可三元组分解。** 这与海岸线无关；landcover 需求恰好把某个需求点
放在了这类不规则点旁。

### 37.5 §36 中仍成立的部分

- 单种子扫描数据本身（`{16,17,18}`、`60`/`120` 恒定）——测量在未裁剪网格上进行，有效；
- 「规则区可行、不规则区不可行」的定性结论——只是「不规则」的来源由海岸线更正为
  **二十面体奇点 + 嵌套过渡带**。

§36 中所有基于 `result` 网格的距离与计数（`19,005` 边界点、`4,318` 个价数-5、
`9.9 km`、`4,146 km`）**全部作废**。

Case 9 状态仍为 `INCOMPLETE`。


## 38. 周界模 3 是各向同性生长的守恒量；修复距离为 2 个面（2026-08-01）

### 38.1 静态可分解性判定：`N = 1`

不再跑流程，直接对每个需求面判定：取其三个 M 角点为候选种子，量各自 `rad3` 足迹经凹角闭合
后的周界长度模 3。新增 `seed_footprint_perimeter_length`（`method_c_perimeter_selection`），
生产路径经 `EARTHMESH_M0_FACE_DEMAND_DUMP_DIR` 导出。

| pass | 需求面 | 候选种子 | 周界分布 | 无可分解种子的需求面 |
|---:|---:|---:|---|---:|
| 1 | `40` | `120` | `{(18,0): 120}` | **`0`** |
| 2 | `8` | `24` | `{(15,0):3, (16,1):11, (17,2):4, (18,0):6}` | **`1`** |

pass 1 **完全规则**：`120` 个候选种子无一例外闭合于 `18`。pass 2 有 `15/24` 落在 `16` 或
`17`，其中 `lon -69.457, lat -55.238` 一个需求面的三个种子全为 `(16, 1)`。

### 38.2 不规则性由 pass 1 自己制造

在裁剪前网格 `tmpfile/*_refine_raw_tri.nc4` 上定位不规则点，并量到 pass-1 需求点的距离：

| 类型 | 数量 | 到最近 pass-1 需求点 |
|---|---:|---|
| 价数 `7` | `126` | **`126/126` 在 `200 km` 内**，中位 `102 km` |
| 价数 `5` | `137` | `126` 个在 `200 km` 内，**仅 `11` 个在别处** |

规则二十面体只有 `12` 个五边形奇点、不产生价数 `7`。**`263` 个不规则点里约 `252` 个是
pass-1 过渡带的产物。**

### 38.3 重排的反事实：把需求投影回规则基底

据此提出：**推迟全部过渡带，各层选择都对未动过的基底做，最后一次性 materialize。**

`case9_demand_projected_onto_regular_base` 在 NXP=243 基底（`nmd=590493`、`nwd=1180981`）
上重测：

| | pass-1 产物上 | **规则基底上** |
|---|---|---|
| pass-1 需求种子 | `120/120` 合格 | `40/40` 合格 |
| pass-2 需求种子 | **`15/24` 不可分解** | **`8/8` 合格** |

§38.1 那个三种子全废的点在基底上是 `(18, 0)`。**污染确实存在，重排确实消除它。**

### 38.4 但合并才是约束

累积区域在**未被污染的规则基底**上仍不合格：

| 区域 | 面数 | 分量 | 周界长度 | 不可分解 |
|---|---:|---:|---|---:|
| pass 1 | `1406 -> 1437` | `20` | `18 x 15`, `23`, `26`, `29`, `32`, `37` | `5` |
| pass 2 | `370` | `6` | `18 x 4`, `22`, `22` | `2` |

长度 `18` 的全是**未合并**的孤立种子；一旦足迹合并，长度跳到 `19`–`37` 的任意值。
其中 `22` 是 §34 记录的失败周界长度，`40` 是 §35 nochile 实验的失败长度——
**在规则基底上复现了生产环境的失败长度**。

`merged_footprint_perimeter_remainders` 量化合并行为（锚点须自身闭合于 `18`，否则每个配对
都继承其缺陷）：

| 情形 | 计数 |
|---|---|
| 两足迹分离（`2` 分量） | `1145` 个 `0` 坏 / `165` 个 `1` 坏（`165` = 五边形邻近种子，占 `11.5%`） |
| 两足迹融合（`1` 分量） | `113` 个，**`60` 个可分解（`53%`）**，长度 `19`–`30`，三种余数俱全 |

**合并约为抛硬币，不是系统性失败。** Case 9 那 `7` 个合并分量全坏是小样本（约 `0.5%`）
叠加本测量的粗糙区域构造（最近 M 点取种子，未走生产的 `mrow` 展开与
`method_c_perimeter_repair`）。

### 38.5 守恒律：各向同性生长保持周界模 3

`case9_bad_component_local_growth` 只生长不可分解的分量，每轮重测全部分量：

| step | 面数 | 分量 | 周界长度 |
|---:|---:|---:|---|
| 0 | `370` | `6` | `22`, `18`, `18`, `18`, `22`, `18` |
| 1 | `470` | `6` | `28`, `18`, `18`, `18`, `28`, `18` |
| 2 | `594` | `6` | `34`, ... |
| 3 | `742` | `6` | **`40`**, ... |
| 4–7 | `914`–`1574` | `6` | `46`, `52`, `58`, `64` |

**每长一圈周界恰好 `+6`，余数恒为 `1`。** `6 = 0 (mod 3)`：在规则三角格上，
**各向同性生长把周界模 3 作为守恒量**。step 3 的 `40` 即 §35 nochile 的失败长度——
`22` 与 `40` 是同一不变量类的不同尺寸。

这一举解释了此前全部观察：

| 观察 | 解释 |
|---|---|
| §6.1 固定缓冲无效且在别处造新违规 | 缓冲各向同性，模 3 守恒 |
| §32 四个绕行开关停在同一处 | 开关改尺度不改余数类 |
| §35 `1` 个需求面也失败于 `40` | `40` 与 `22` 同类 |
| §36 inward-growth 两圈后恢复 | 那里是并入被触及 M 点的足迹，**各向异性** |

**此前每一次「扩大范围」都在一条模 3 恒定的轨道上滑动。失败不是搜索得不够远，
而是搜索方向被守恒律封死。**

### 38.6 修复距离：`2` 个面

`case9_bad_component_single_face_edits` 枚举全部单面添加（每分量 `50` 个候选）：

| 分量 | 基准 | 单面可达 | 达到余 `0` |
|---|---|---|---|
| `76` 面 | `22` | `{(22,1):11, (23,2):18, (26,2):4}` | **否** |
| `78` 面 | `22` | `{(22,1):11, (23,2):22}` | **否** |

单面添加只能到余 `1` 或 `2`。`case9_bad_component_minimal_repair_depth`
（宽度 `24`、深度 `4` 的 BFS）继续搜：
...
**两个分量都在加 `2` 个面后可分解——占分量的 `2.6%`。**

| 操作 | 周界变化 | 能否改余数 |
|---|---|---|
| 均匀生长一圈 | 恒 `+6` | **不能**（守恒） |
| 加 `1` 面 | `0` / `+1` / `+4` | 只到余 `1` 或 `2` |
| **加 `2` 面** | **`+2`** | **到余 `0`** |

### 38.7 对不可满足性结论的影响

`docs/case9_infeasibility_report_2026-07-31.md` 的结论在**其自身声明的域内仍然成立**——
该域内全部算子（缓冲、退让、丢弃、支撑预言器、求解器宽度）都是各向同性或不改变区域形状的。

但域外存在一个 `2` 面的解。**Case 9 的正确表述不是「不可满足」，而是
「现有修复算子的方向被模 3 守恒律封死」。**

### 38.8 尚未验证的部分

- 本节全部测量在**规则基底**上、用**粗糙区域构造**（最近 M 点取种子，无 `mrow`
  展开、无生产 `repair`）。真实 pass 2 跑在含 `126` 个价数-`7` 点的 pass-1 产物上，
  `2` 面补丁未必同样有效。
- **周界可三元组分解只是九条合法性约束之一。** 补丁之后价数 `<= 7`、过渡片可实现、
  层级平衡都需重验。
- 加面只增不减，覆盖是超集，**不违反 hard demand**；但这未经生产路径确认。

新增只读测试：`case9_demand_projected_onto_regular_base`、
`merged_footprint_perimeter_remainders`、`case9_bad_component_local_growth`、
`case9_bad_component_single_face_edits`、`case9_bad_component_minimal_repair_depth`
（`tests/method_c_single_seed_perimeter.rs`）。`method_c_selected_face_components`
放开至 `pub(crate)`。`earthmesh_mesh` lib 测试 `167` 通过。

Case 9 状态仍为 `INCOMPLETE`；生产路径未改动。


## 39. 加宽修复搜索：模 3 墙被突破，可实现性接替成为约束（2026-08-01）

> **[更正 2026-08-01]§39.3–§39.5 的对比无效。** 两次 beam 运行**未设置**
> `EARTHMESH_M0_CROSS_LEVEL_SUPPORT=1`，而基线设置了。因此「materialize 次数 `3` vs `1`」
> 与「支撑预言器无轮次」并非 beam 造成，而是跨层支撑整个机制未启用。
> §39.1、§39.2 的改动描述与「三元组失败消失」的观察不受影响——那是同一份 beam 代码在
> 两种配置下都成立的结果。带正确配置的重测见 §40。


### 39.1 现有修复本就是各向异性的，但**贪心**

`try_grow_method_c_non_triplet_perimeter_once` 逐个试加单面、闭合凹角、量周界，按
`(加了几个面, 余数分, 周界长)` 取**唯一最优**，外层 `repair_method_c_non_triplet_perimeter`
最多迭代 `12` 轮。

它不是各向同性缓冲——但每轮只保留一个候选。§38.6 实测：从余 `1` 出发，单面添加只能到
`{22, 23, 26}`（余 `1`/`2`），**必须两面才到余 `0`**。贪心因此可能提交到一个「修复不在其后」
的首步。

### 39.2 改动：宽度受限的束搜索

`method_c_perimeter_repair_grow` 重构为：

- `method_c_repair_candidate_faces`——候选枚举（原逻辑抽出）；
- `method_c_scored_repair_trials`——逐候选评分，`support_aware` 开关决定是否在接受
  可分解掩码前检查父层支撑；
- `try_grow_method_c_non_triplet_perimeter_once`——**行为逐字不变**；
- `try_grow_method_c_non_triplet_perimeter_beam`——每步保留最优 `width` 个候选，最深 `depth`；
- `try_grow_method_c_non_triplet_perimeter`——分发入口，两处调用点改接于此。

`EARTHMESH_M0_REPAIR_BEAM_WIDTH` 默认 `1`（走原贪心路径），`EARTHMESH_M0_REPAIR_BEAM_DEPTH`
默认 `3`。`earthmesh_mesh` lib 测试 `167` 通过，默认路径无回归。

### 39.3 生产实测：三元组失败消失

NXP=243、`max_level=3`、`width=8`、`depth=3`：

| | 基线（贪心） | beam | beam + 支撑感知 |
|---|---|---|---|
| materialize 次数 | **`3`** | `1` | `1` |
| 支撑预言器轮次 | `39 -> 5` | **无** | **无** |
| repair 进度行 | `14` | `22`（`64/64` 耗尽） | `22`（`64/64` 耗尽） |
| 失败 | 三元组 `[48, 54, 18, 18, 22]` | 父边界跨越 | 父边界跨越 |
| 失败位置 | `(-71.974, -52.473)` | `(-74.199, -50.446)` | `(-74.199, -50.446)` |
| 耗时 | `3:49` | `~5:30` | `~25 min` |

**`[48, 54, 18, 18, 22]` 的三元组失败不再出现**——beam 确实在生产路径上找到了可分解周界，
§38.5 的模 3 守恒律不再是障碍。

### 39.4 但可实现性接替成为约束，且 beam 走得更近

两个 beam 运行都：

- 只完成 `1` 次 materialize（基线 `3` 次）；
- repair 循环耗尽 `64` 次尝试；
- 失败于**同一经纬度** `(-74.199, -50.446)`，仅相邻面 id 不同（`519896` vs `519869`）。

**支撑感知筛选改变了失败在哪个面，没有改变是否失败。**

### 39.5 结构性原因：修复晚了一层

支撑预言器 `required_parent_support_lineages_from_target_levels_and_face_demands`
（`global_source.rs:1192`）是**跨 pass 的前置预测**——它从目标层级与需求推断父层需要细化
哪些面，运行在 repair **之前**。

因此：**当 repair 发现周界不可分解时，父层已经定型。** beam 能在当前 pass 内找到可分解
掩码，但那些掩码所需的父层支撑不存在，而 repair 无法回头要求父层重做。

这解释了为什么 beam 反而更差：它选出的掩码在可分解性上更优、在可实现性上更差，而贪心
选出的掩码虽不可分解，却让流程走到了支撑预言器可以介入的下一轮。

### 39.6 下一步方向（未实施）

已有回灌通道：`take_post_drop_support_lineages`（`global_source.rs:1206`）把退让后新增的
支撑需求折回预言器。**同一通道可以承载 beam 的未支撑见证**——即 repair 找到可分解但
不可实现的掩码时，不是丢弃它，而是把其见证面登记为父层支撑请求并重跑父 pass。

这不是针对本例的调参，而是补上「repair 的掩码选择无法影响父层」这条结构缺口。

### 39.7 状态

- 模 3 守恒律：**已在生产路径突破**（三元组失败消失）；
- 可实现性：**接替成为约束**，beam 未解决且比贪心更早失败；
- 默认行为：**未改变**（`width=1`），`167` 个测试通过；
- beam 为默认关闭的可选诊断。

Case 9 状态仍为 `INCOMPLETE`。


## 40. Case 9 闭合：见证面回灌 + 加宽搜索（2026-08-01）

### 40.1 改动

两处，互相依赖，缺一不可：

1. **加宽修复搜索**（§39.2）——`EARTHMESH_M0_REPAIR_BEAM_WIDTH`，默认 `1` 即原贪心。
   突破 §38.5 的模 3 守恒律。
2. **见证面回灌通道**——beam 找到「可分解但父层未支撑」的掩码时，不丢弃，而是把其
   未支撑见证面经 `record_post_drop_support` 登记；`global_source.rs` 的 pass 循环
   捕获 materialize 失败，若携带新见证面则折入父层 `support_lineages` 并回退重建，
   否则原样报错。`added == 0` 守卫防止空转。

结构缺口在于：支撑预言器 `required_parent_support_lineages_from_target_levels_and_face_demands`
是**跨 pass 前置预测**，跑在 repair 之前。repair 发现周界不可分解时父层已定型，
**repair 晚了一层**。回灌通道是唯一能让 repair 的掩码选择反向影响父层的路径。

### 40.2 结果

配置：NXP=243、`max_level=3`、原生 `86400x43200` landcover、
`EARTHMESH_M0_CROSS_LEVEL_SUPPORT=1`、`BEAM_WIDTH=8`、`BEAM_DEPTH=3`。

| | 默认（beam 关闭，对照） | **beam + 回灌** |
|---|---|---|
| 退出码 | `2` | **`0`** |
| 失败 | `perimeter lengths [48, 54, 18, 18, 22]` | **无** |
| 产物 | `0` 个 | `gridfile_NXP0243_tri.nc4` 等 |
| 质量门 | — | **`verdict: pass`** |
| 单元 | — | `842,241` |
| 单元面积比 | — | `18.215` |
| 最小边长 | — | `6.559 km`（基底 `32.9 km`） |

**对照跑使用同一个二进制、同一份配置，唯一差别是两个环境变量**，失败与改动前的原始基线
逐字相同。三元组失败精确地随 beam 开关出现与消失。

### 40.3 回灌收敛

```
pass 2 requested 39 support        -> 回退
pass 2 requested 5 support         -> 回退
pass 2 folding 122 见证面          -> 回退      <- 新通道
pass 2 requested 5 support         -> 回退
pass 2 folding 67 见证面           -> 回退      <- 新通道
pass 3 requested 12 support        -> 回退
pass 2 folding 44 见证面           -> 回退      <- 新通道
pass 3 requested 12 support        -> 完成
```

见证面数 `122 -> 67 -> 44` 单调收敛。基线死在第三行的位置。

### 40.4 需求全部服务完毕

`EARTHMESH_M0_FACE_DEMAND_DUMP_DIR` 逐 pass 导出：

| pass 3 访问 | 需求 | 结果 |
|---|---:|---|
| 第 1 次 | **`1`**（`face 522244`, `lon -72.995 lat -46.031`，种子周界 `[(16,1),(18,0),(21,0)]`） | 请求 `12` 个父层支撑 -> 回退 |
| 第 2 次 | **`0/1189946`** | `spawn` 返回 `None`，收敛退出 |

父层带着新支撑重建后把该处又细了一层，该面变成纯海或纯陆，`th_sea_ratio` 区间不再满足。
**这是 landcover 判据自终止的设计行为，不是丢弃。**

最终网格 `earthmesh_m_refine_level` 分布 `{0: 838759, 1: 3281, 2: 201}`，
`earthmesh_m_refine_level_orig` 分布 `{0: 839594, 1: 2494, 2: 153}`，
**`orig > achieved` 计数为 `0`**——无任何单元被降级交付。

> **Case 9 的 `48` 个硬需求面全部服务完毕，剩余需求为 `0`。**
> 不是 `47/48`，不是 §29 的 `93%`。

### 40.5 确定性

两次独立运行（`case9-beam-feedback-1785556817`、`case9-beam-verify-1785561669`，后者额外
开启需求导出）产出的 `gridfile_NXP0243_tri.nc4` **SHA256 逐字节相同**
（`0fbbe0e31d27ee05...`），层级分布与质量指标全部一致。

### 40.6 完整因果链

| 环节 | 内容 | 章节 |
|---|---|---|
| 症状 | `[48,54,18,18,22]` 不可三元组分解 | §34 |
| 为何修不好 | 周界模 3 在各向同性生长下守恒（每圈 `+6`），历史上全部算子皆各向同性 | §38.5 |
| 为何贪心不行 | 从余 `1` 单面添加只到余 `1`/`2`，**需两面**，贪心会走过头 | §38.6 |
| 突破口 1 | 宽度受限束搜索 | §39.2 |
| 新瓶颈 | 可分解掩码父层不支持 | §39.4 |
| 为何 repair 解决不了 | 支撑预言器是跨 pass 前置预测，**repair 晚了一层** | §39.5 |
| 突破口 2 | 见证面回灌通道 | §40.1 |
| 结果 | `122 -> 67 -> 44` 收敛，需求归零 | §40.3–40.4 |

### 40.7 状态与未尽事项

- **默认路径未变**：`BEAM_WIDTH` 默认 `1` 走原贪心，对照跑确认失败逐字相同；
  `earthmesh_mesh` lib 测试 `167` 通过。
- **性能**：beam + 支撑判据当前约为贪心的 `20x`（`3:49` -> `76 min`）。主因是每个候选面
  要做约 `7` 遍 `nwd = 1,180,981` 的全局遍历，而被修的分量只有 `76`–`78` 个面——
  **算法是局部的，数据结构是全局的**。已完成的分配优化（只为幸存者建掩码、复用 scratch
  缓冲）砍掉分配，未砍扫描。
- **未做**：把扫描限制在分量邻域（需改 `close_method_c_concavities_for_level_with_neighbors`、
  `method_c_perimeters_from_selected_faces` 等公共 helper 的签名）；在此之前不宜把 beam
  设为默认。
- **未做**：Case 9 之外的回归（其他 NXP、其他产品、区域网格）。

Case 9 状态：**`RESOLVED`**（限定于 §40.2 所列配置与 308a8163 路径）。

> **当前 HEAD 回归限定（2026-08-01）**：§40 的两次成功产物生成于后续性能/周界扫描提交之前。在 `d8683f3b` 及之后修改继续叠加时，必须重新跑整网 Case 9 E2E，确认 hard demand 归零、质量门通过、并与 §40.5 的旧成功产物哈希对比后，才可声明当前 HEAD 仍闭合。
