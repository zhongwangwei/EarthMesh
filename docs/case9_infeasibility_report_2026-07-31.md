...
>
> **根因定位到海岸边界**：单种子扫描显示未裁剪均匀基底上 `77.5%` 以上的种子给出可分解
> 周界（长度 `18`）；而 `ocean mask` 裁剪把规则的价数-`6` 拓扑打成 `19,005` 个边界点
> （其中 `4,318` 个价数为 `5`），失败面距最近海岸边界点仅 `9.9 km`（不到一个单元）。
> **landcover 需求按定义全部落在海岸线上，因而全部落在不规则边界区。** 详见研究文档 §36。
