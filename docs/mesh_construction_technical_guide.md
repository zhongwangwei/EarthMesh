...
### 11.4 多重细化战役(2026-08-06):九轮全球验证的机制地图

目标:让点+半径路径在全球尺度正确细化。九轮真实工程验证(每轮约 35 分钟),
...
| 诊断基础设施 | 40 分钟/轮的全球验证无法迭代 | 被拒组落盘 `refused_groups.jsonl`,秒级本地重放(同上) |
...
at M point 61085`),pass 1 下该错只可能由 `mrlu < 1` 触发 —— **裙边上存在
mrlu=0 的边**,即发射重建漏设边代的具体线索。下一步:把成功组也落盘,本地全
序列重放即可免 50 分钟全球验证。
