...
3. **栅格尺寸**（2026-08 起由 Project lowering 推导）：h 场在**三角形中心与边中点**处被采样，因此第 2 点的"间距 ≤ h"不是建议而是硬约束。栅格过粗时 `level_at` 采出的是混叠后的锯齿边界，Method-C 收到不合法掩膜，直到 `perim_fill3` 深处才报 `transition patch has no solid split edge`。
   ...
   **点+半径原型（2026-08）受阻记录**：尝试绕开栅格、把海岸需求表达为一批 `circle` 区域走几何路径，未能跑通。连续撞上五个互相牵制的约束：陆海判据是 `landtype != 0` 而非阈值 12（后者是细化判据）；`refine_cal=.FALSE.` 时 `mask_refine_cal_type` 仍被检查；`max_iter_cal=0` 触发 1..=5 越界；改回 1 又让 calculated 分支复活；namelist 写对后运行时行为仍不符预期。这说明改用点+半径不只是换需求表示，还要面对几何路径盘根错节的开关语义——h 场路径的引入可能部分正是为绕开它。正确的验证方式是写最小 Rust 测试直接调 `spawn_nest(&regions, ...)`，绕开 namelist 层，那也能进回归套件。
