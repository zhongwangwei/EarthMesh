...
| + 移点，朝细侧，有改善门控 | **20** | **2.184** | 跑满 40 轮 |

**结论：这个实现让情况变差，且阻止了收敛。** 移动被硬门控接受但没让全局变好，而它一旦
提交，驱动就把「有东西被接受」读成进展，于是永远不触发 `NoAcceptedTransactions`。改善
门控（§13.3 的离散版）是逐站点局部的：局部最差比下降可以伴随全局恶化。所以接线撤掉，
原语保留——它们本身正确，`propose_move` 是公开且有测试的，缺的是一个全局有效的目标。
