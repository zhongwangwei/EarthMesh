...
#### 已落地,以及昨天那句判断错在哪(2026-08-20)

翻译已合入 `emit_method_c_tables`,阶梯的门从 `im <= self.nmd` 变成直接用翻译后的
父 id。一圆复现由拒绝转为成功并通过拓扑校验。

**2026-08-19 我写的"它把案例带过了 Canonical 视为致命的墙"是错的表述。** 查实之后:
**没有任何 Canonical 拒绝的案例变成接受**,两个一致性测试都仍然拒绝。变的是哪道检查
先抓到它,而且**两个方向都有**:

| 案例 | 修前报的错 | 修后报的错 | 方向 |
|---|---|---|---|
| 双层走廊 | 价数 | 三元组分组 | 诊断变糊 |
| 混合区域跨 mrow | 三元组分组 | 价数 | 诊断变准 |

第二行尤其说明问题:`spawn_nest_rejects_mixed_regions_when_child_crosses_parent_mrow_like_canonical`
当初是靠三元组消息尾部那句附带的 "without crossing the parent boundary" 通过断言的,
而修后报的价数消息说的正是这个测试名字所指的约束。**两个测试都是在对错误文本做字符串
匹配,于是各自被钉在了"当时恰好浮上来的那个错误"上**——这本身是一类脆弱断言,
和 §11.49 那条"只在测它的那台机器上成立的断言"同科。

两处断言的调整依据分别写在测试里:混合区域那条是把只认两种措辞的列表补成三种(走廊那条
一直就同时接受这三种),走廊那条丢掉的是"必须在三元组之前拒绝",因为那个"之前"只在
修复被类型混用禁用时才成立。

**代价:Method-C 的 lib 套件从 29 秒涨到 192 秒**,全部集中在走廊那一个案例——阶梯
越过价数墙之后要在三元组上再磨约 50 轮。

修复循环的预算随后从 64 降到 **32**(实测:419 次成功修复中 411 次 1 轮、7 次 2 轮、
最深 14 轮;而失败的 pass 会跑到 **57**,逼近旧上限)。**套件回到 146 秒,只省了 24%,
远不及预期。**

原因值得记下来:**开销的主因不是轮数,是掩膜在长。** 走廊案例实测从 90 个面涨到 915,
每一轮的 emit 都比上一轮贵,所以截掉尾部轮数只削掉了一部分。把上限再压到 16 会紧贴那个
14 轮的成功案例,不划算。

另有一条否定结论:**掩膜增长倍数不能用来判成败**,因此"长太多就收手"这条规则不成立。

#### 顺带查清:修复并没有在悄悄放大细化区域

"阶梯为修周界而扩张掩膜"这件事本身值得怀疑——**一次成功的 spawn 如果多细化了几倍的
单元,交付的就不是用户请求的网格,而它会通过所有质量检查**,正是 §11.1 那一类。
在同一次调用内同时记录首末面数(不做事后配对)得到:
...
| 掩膜被修复扩张过的 | **6 个(1.4%)** |
| 扩张倍数 | 中位 2.0×,最大 22.5× |
| 那几个的终点面数 | 45、45、54、54、54 |

**不是缺陷。** 终点全部落在 45–54,而本节写作提醒里记的正是"细化面积 = 覆盖种子数 ×
54 面":一个只覆盖 2 个面的请求必然被凑成一个可行的 Method-C 块。98.6% 的成功 pass
掩膜一个面都没长。

先前一版分析给出"成功案例最大增长 65 倍"是**解析错位**造成的——它按 `i==1` 切分
run,而多个 pass 的计数器都从 1 开始,于是把一个 pass 的起点和另一个 pass 的终点配了
对。教训与本轮 `--nocapture` 那次同科:**仪器没问题,读法有问题**。

#### 生产算例上零影响(2026-08-20 真实数据 A/B)

合成 sweep 只能说明构造出来的圆变好了,回答不了"生产几何上安全吗"。用真实数据做了
一次 A/B:配置取生产原样的 `examples/default/land_hex_global.nml`(NXP 80、
`niter = 5000`、125 MB 真实 IGBP 栅格、`refine_num_landtypes` 判据),只改两处——
后端换成 `method_c`、输出目录换到临时盘。改动局限在 `earthmesh_refine_method_c`,
所以在同一棵树上把该 crate 在 `cbc8ce2` 与 HEAD 之间来回切换、各跑一次。

| | |
|---|---|
| 三份 gridfile(result / tmpfile raw / gridinit) | **校验和逐字节相同** |
| 运行日志(路径归一化后) | **一行差异都没有** |
| 两侧 | `exit=0`,`refine_landtype_masked_cells=36932` |

**价数修复路径在这个算例上根本没触发。** 周界修复只在掩膜周界出问题时才跑,这个算例的
几何没走到那里。所以那 18 个被救回的配置是**真的边界情形**,而这次改动对生产输出的影响
是**零而不是"很小"**——这是它可以放心留在树上的关键证据。

反过来说,这也意味着**生产算例证明不了这个修复有用**:它的价值只在那些原本会被整组拒绝
的边界几何上,而那正是 §11.5 开头那个海岸带失败的形状。

#### 隐患曾只有注释,后来上了测试(2026-08-19)

这轮的结论是"两个方向都改变交付行为,先不改"。但**那 379 次错位修复对测试套件原本
完全不可见**——我自己就差点把这道门当死代码删掉,而唯一的信号是走廊测试挂掉并报一句
与病根无关的话。所以补了两处,都不改行为:

- `tests/method_c_valence_id_space.rs` —— 用那个一圆复现钉住 `im > nmd`,把 id 空间
  不一致变成可执行断言。注意它是**单向**的:只能在 id 确定不属于父空间时报警,对
  "子 id 恰好落进父范围"这个真正被阶梯拿去用的情形无能为力,那个只能靠本节的计数。
- 走廊那个一致性测试加了文档注释,写明它**顺带**守着这道门,以及挂掉时该先读哪一节。
...
### 11.70 Method-C 过渡模板拒绝的周界形状，与按块容错（2026-09-25）

**规律（实测）。** `perim_fill3` 按“每块周界的连续三元组”缝合过渡带，只能处理
直边和“开启一个三元组”的拐角。`method_c_shape_probe` 在 NXP 21/40 上用 420 个
连续足迹并集跑生产路径（凹角闭合 → 三元组规范化 → emit → 修复阶梯），下面这条
规则判断成败**零误差**（`method_c_perimeter_shape_violations`）：

- 周界上没有缺陷点（五边形，或上一代过渡带留下的非六邻点）；
- 没有单面尖刺（只被 1 个已选面持有的周界点）；
- 每个拐角（已选面数 ≠ 3）都在三元组第 0 位——等价地，两拐角之间的直边长度是 3 的倍数。

数字：生产路径在第一代上的成功率 93–100%（8 月“9 种子 21%”没有经过凹角闭合、也
没有 emit，不代表生产）；周界经过五边形的 5 例**全部失败**，不经过的 415 例失败 2
例且恰好都有错位拐角；**修复阶梯救回 0 例**。Case9（全球海洋海岸阈值）第 3 代：规则
准确标出德雷克海峡由规范化引入的单面尖刺——一个尖刺给周界多插一个点，其后所有拐角
都错一位，所以一个面能让整块失败。

**修不动的部分（否定结果）。** 局部改掩膜救不回这些形状：在违规点附近加单面或整块
足迹、把五边形 0–4 环的足迹整块吞入、或清掉其 0–4 环的面，都没有一例建成——stride-3
的格点相位绕五边形一圈闭合不了。把相位修复接成修复阶梯的第一级还会让失败路径跑几
分钟。规则会把 Canonical 一致性测试里**能建成**的掩膜也标为违规，所以它只能指导修复，
不能拿来拦截 emit。

**按块容错（`spawn_nest_pass_dropping_unbuildable_blocks`）。** h-field 的某一代失败
且错误是可修复的闸门时，不再整代失败，而是丢掉一个连通细化块后重试：先逐块单独试建，
...
### 11.71 Red-Green 三角形模式为什么大面积过度细化（2026-09-25）

1a8d266f/ae823600 之后，Red-Green 网格第一次带上每单元实际层级，质量报告的目标/实际对账
随之显出：全球海岸案例（Case9，RedGreen、h-field 关）685,065 个单元中 428,624 个比目标
细，其中 231,895 个根本不在任何需求圆内（目标 0）却到了第 3 层；过度细化离最近的同层圆边缘
中位 5.3 个圆半径、最远 52 个（约 1,300 km）。按层级分组的面积中位数之比 4.03/4.03/4.02，
层级本身无误。

**计数（`conforming::refine`，逐轮逐波）：** 被标记的面全都在目标深度减 1（前一轮 halo 裁剪
所致），“深度低于要求”的拆分为 0；扩出的拆分 100% 来自闭合 `needs_red`。第 3 轮标记
52,365 个，闭合再拆 157,720 个——约一半是“两条以上悬挂边”（78,606），一半是“green 二分角度
低于下限”（78,480）。

**根因：green 下限过紧。** 下限取红叶网格最小角的一半（这里 52.62°/2 = 26.31°）。green 二分
把顶角连到对边中点，并不平分顶角，55° 的顶角可分成 25° 与 30°，于是大量普通三角形被拒绝、
改作红色拆分；红色拆分又给外圈留下悬挂点，触发“两条悬挂边”，一波波外扩。下限改为 20°（或
0°，结果相同——此网格上 green 子三角形最小角本就 ≥ 20°）：角度拒绝归零，“两条悬挂边”从
7.9 万降到约 1,600；单元 685,065 → 231,501，过度细化 428,624 → 14,887，面积变异系数
2.15 → 1.06，最小角 26.31° → 24.65°。

**被掩盖的第二个问题：需求不逐层嵌套。** 每一轮只用本层规划出的圆打标记，第 L 轮的标记必须落在
第 L−1 轮细化区的 halo 内，否则取消。判据圆按层独立规划、不保证嵌套，过度细化曾把上一层撑大
而掩盖了它；下限放宽后，第 3 轮被取消的标记从 45,670 增至 61,010，细化不足 32,832 → 44,975
（差一层以上 6,351 → 9,929）。

**修复方向（会改变 Red-Green 输出，未实施）：** 1）green 下限改为固定值或可配置，而不是
“最小角的一半”；2）第 L 轮打标记时并入更深层的需求圆（与命名区域 `level() >= L` 的做法一致），
让深层需求必有上一层承托。两者需一起做：只做 1）会以细化不足换过度细化。

**2026-09-26 补充：用 h-field 驱动后的实测（Case9，海洋 Tri FVCOM，g=0.2，切割后计数）。**
a8083510 让 Red-Green 能按 h-field 的目标层级标记。h-field 天然嵌套（某点要第 3 层即也要第 1、2 层），
halo 取消因此归零（三层均为 0），11.71 的第二个问题不再出现。green 下限用实验开关覆盖（未提交）：

| 需求 / green 下限 | 单元 | 比目标细 | 比目标粗 | 最大层差 | 面积 CV | 角度（修复后） |
|---|---|---|---|---|---|---|
| 判据圆（现状）/ 26.31° | 685,182 | 428,629 | 32,829 | >1 | 2.15 | 35.21°–84.86° |
| h-field / 26.31° | 151,427 | 27,641 | 262 | 2 | 0.69 | 35.25°–84.75° |
| h-field / 20° | 131,752 | 2,812 | 402 | 1 | 0.56 | 35.25°–84.75° |

两组 h-field 运行的物理 2:1 违例均为 0，耗时约 3 分钟。对照 Method-C 同配置：137,925 个三角形，
但丢弃 3 个块、19.4% 需求未满足（11.70）。即 Red-Green + h-field + 20° 下限在同样单元数下满足了几乎
全部需求。20° 下限产生的更差初始角（24.65°–101.34°）由 11.73 的角度修复拉回窗口，未见困难。

**已实施（4c，2026-09-26）：** 需求接口新增 `nests_by_construction()`（h-field 为真，判据圆与命名区域
为假）；Red-Green 三角形模式遇到天然嵌套的需求时，green 下限固定为 20°
（`RedGreenSettings::green_floor_deg`，`redgreen_bridge::NESTED_DEMAND_GREEN_FLOOR_DEG`），否则保持
推导下限。判据圆路线输出不变：它不嵌套，那里的过度细化在承托深层需求，放宽只会换来更多细化不足；
待给判据圆补上逐层嵌套后再统一。GUI 中选择 Red-Green 时，默认需求表达改为 h-field（标“推荐”），
用户已选的路线不动。

### 11.72 三角形内角强制窗口 35°–85°（2026-09-25）

**规则。** 全三角形网格的每个内角必须落在 [35°, 85°]（`earthmesh_quality::TRIANGLE_ANGLE_WINDOW_DEG`），
否则 `min_angle_deg` / `max_angle_deg` 门为 Fail、质量结论为 Fail。这是强制的：namelist 与项目的
`min_angle_deg` 都不能放宽它。多边形（Hex）单元内角约 120°，不受此窗口约束，仍按原来的门判断。
Fail 之后按项目策略：`Block` 与 `AutoRefine` 拒绝交付，`Warn` 仍交付但结论为 Fail。
`max_angle_deg` 失败被视为 AutoRefine 不可修复（质量报告不为它列出可修复单元），直接报错。

**现状（实测）：** Method-C 全球海洋 / 陆地 Tri 为 37.06°–83.21° / 37.36°–83.37°，通过；Red-Green
全球海岸原为 26.31°–100.78°，失败；加了 11.73 的角度窗口修复后为 35.21°–84.86°，通过。

**为什么 Red-Green 过不了，而 AutoRefine 加密也修不好。** 1→4 红色加密的子三角形与父三角形相似，
角度原样保留：坏角既不会被制造，也不会被加密修好。坏角来自层与层之间的 green 闭合：从对角顶点连到
悬挂中点把三角形劈成两半，等边三角形劈出 30°/60°/90°，所以只要用 green 闭合，每条层级交界线上都有
约 90° 与约 30° 的角。继续加密只是把交界线往外挪一圈，同样的三角形在新位置重现。能解决的是换过渡
构造或做后处理优化：Method-C 的过渡行模板加弹簧平滑落在窗口内；最长边二分（LEPP）保证最小角下界；
Delaunay 翻边与节点平滑均匀化过渡带（Red-Green 现有的弹簧在 Case9 上反而把角度变差到
21.4°–116.8°，被自动撤回）。11.71 的两项修复（固定 green 下限、需求逐层嵌套）只解决过度细化，
不能让 Red-Green 进入这个窗口。

### 11.73 Red-Green 三角形的角度窗口修复（2026-09-25）

**做什么。** Red-Green 三角形模式在 Lawson 抛光之后多一步终端修复
（`earthmesh_mesh::repair_triangle_angle_window`，适配层 `redgreen_bridge::repair_redgreen_angle_window`），
目标窗口是 11.72 的 [35°, 85°] 各向内收 0.25°（坐标写入文件时的舍入不能把角带回窗口外）。按轮次在
坏三角形及其一环邻域上依次做：

1. **度数翻边。** 翻转一条边，若它四个端点的度数离 6 的平方偏差之和下降，且新三角形不低于 15°、朝向
   不变、不超过网格已有的最大度数；平方偏差之和不变但最大偏差下降的翻边也接受（一个 8 变成 7，
   三个 6 各动一步——没有这一条，三层加密的测试网格停在 34.89°–85.08°，13 个三角形卡在度数 7/8
   顶点上，有了它 4 轮进入窗口）。度数 `k` 的顶点平均角为 `360/k`：度数 ≥11 的顶点到不了 35°，
   度数 4 的顶点压不到 85° 以下，怎么移动都没用。每轮末尾对仍在窗口外的三角形的边再做一次
   “按余量”翻边：只要这一对三角形到窗口的最差余量变好就翻。
2. **删除度数 3/4 的加密顶点**，其一环重新三角化（度数 4 取两条对角线中最小角更大的一条）。
   基础网格顶点（id < `num_center`）不删，所以五边形 id 不变；更大的 id 顺次前移。
3. **移动顶点。** 先向一环重心移动（步长 1、½、¼），再做 8 方向模式搜索（步长 0.2h 递减到 0.005h），
   目标是其周围三角形到窗口的最小余量，余量相同时比较负余量之和。每一步只在局部变好时接受，
   开放边界上的顶点不动。

替换出的面取其来源面中最深的细化层级，`earthmesh_{m,w}_refine_level` 因而保持有数。三角形模式下
不再跑区域弹簧：它优化边长而不是角度，在 Case9 上把角度变差到 21.4°–116.8°。

**为什么要三件都做（Case9 全球海岸原型的逐步结果，1,154,284 个三角形，切割前）：**

| 步骤 | 窗口外三角形 | 最小 / 最大角 |
|---|---|---|
| Red-Green 原样 | 58,223（5.04%） | 26.3° / 100.8° |
| 仅按角度翻边 | 不变（网格已近 Delaunay，2 万条边抽样只有 22 条翻边能提高最小角） | |
| 度数翻边 + 重心平滑 | 7,521 | 25.0° / 104.3° |
| + 最差余量模式搜索 | 28 | 30.1° / 90.1° |
| + 删除度数 4 顶点 | 4（差 0.1°） | 34.9° / 85.0° |

剩下的 28 个里 16 个的大角在度数 4 顶点上，正是移动修不了的那一类。

**生产结果（Case9，Red-Green，海洋 Tri FVCOM）：** 切割前 58,532 → 2 个三角形落在内收窗口外，角度
26.31°–100.78° → 35.21°–84.86°；7,954 次翻边、108,635 次移动、删除 9 个顶点、5 轮；物理 2:1 违例
0（最大相邻比 1.749）；最终 685,182 个三角形，`min_angle_deg` 与 `max_angle_deg` 窗口门均通过。
整次运行 221 秒。

**局限。** 修复是局部的，不保证总能进入窗口；做不到时剩下的三角形由 11.72 的门拦下，而不是被放过。
Red-Green 经典过渡行路径（Hex 输出）不经过这一步，Hex 单元本来不受窗口约束。

### 11.74 三角形角度契约：所有算法统一，合格线 35°–85°，目标 60°（2026-09-26）

**用户的要求。** 35°–85° 针对最终网格、针对所有算法，过程怎样不管；在此之上角度越接近 60° 越好。
11.73 的修复只接在 Red-Green 上，而且一进窗口就停（结果贴边 35.25°/84.75°）。

**做法。**
- `earthmesh_mesh::repair_triangle_angle_window` 增加第二阶段。进窗口后分两段：先按“每个顶点周围
  最差的偏离 60°”移动（极端角往回拉），再把窗口收紧到这一段达到的最大偏离，在其中最小化所有角与
  60° 的偏差平方和。只压最差角会把其它角推离 60°（Case9 平均偏离 4.99° → 5.36°），只用平方和会把极端
  角推回窗口边缘（最大偏离 24.75°），四次方和两头都不好；两段式两者兼顾。窗口关于 60° 对称，每一步
  只在局部不越出窗口时接受，所以已在窗口内的三角形不会被推出去。
- 修复放在输出层一处（`refine_pipeline/angle_contract.rs`），在编排尾部、任何后端的结果写出之前，
  对三角形、球面网格执行一次；LEPP 质量后处理的备选网格同样处理。可翻边、删度数 3/4 的非五边形顶点；
  每单元层级、Method-C 谱系/原始层级/`ngr`、五边形 id 按修复报告的来源（`AngleWindowOrigins`）跟随
  行；按原行布局写回；三角形中心按后端原约定（外心或形心）只重算改动过的面。改动了网格时，运行记录
  改按最终网格计数，不再沿用修复前的 Voronoi 表。不增加任何顶点的最大度数（模型格式有上限）。
- Red-Green 自身的末端修复只保留“进窗口”，往 60° 推交给这一处统一步骤。
- CMRC 走单独流水线，不经过这一步；它的证书保证 40°–80° 或 38°–82°。Hex 输出不受三角形窗口约束。

**实测（统一步骤的日志：修复前 → 修复后）：**

| 样本 | 角度 | 最大 \|角−60\| | 平均 \|角−60\| | 耗时（整次运行） |
|---|---|---|---|---|
| LEPP 全球 Tri（`lepp_tri_global`） | 28.71°–117.09° → 43.15°–76.85° | 57.09° → 16.85° | 4.43° → 3.80° | 5.5 s |
| Method-C 海洋 Tri g=0.05（447,714 单元） | 37.06°–83.37° → 44.74°–75.26° | 23.37° → 15.26° | 4.39° → 4.07° | 约 240 → 303 s |
| Method-C 陆地 Tri g=0.05 | 同上（同一全球网格） | 23.37° → 15.26° | 4.39° → 4.07° | 295 s |
| Red-Green + h-field（131,864 单元） | 35.17°–84.75° → 37.40°–82.60° | 24.83° → 22.60° | 4.99° → 4.49° | 217 s |
| Red-Green 判据圆（685,228 单元） | 35.24°–84.75° → 36.57°–83.43° | 24.76° → 23.43° | 5.86° → 5.37° | 约 250 → 530 s |

所有样本的拓扑门（悬空边、朝向、邻接度数）通过；LEPP 从质量判定失败变为通过。

**发现：** LEPP 的三角形网格本身有度数 7 的顶点（修复前 9 个，修复后 6 个），ICON 格式最多容纳 6，
所以 LEPP → ICON 交付仍失败——这是 LEPP 输出的性质，此前被角度失败挡在前面没有暴露。

**局限与待做：** 第二阶段在大网格上耗时明显（Red-Green 判据圆 68.5 万单元多出约 280 s）；Red-Green
网格的最大偏离只从 24.8° 降到 22.6°–23.4°，受其过渡带拓扑所限。
