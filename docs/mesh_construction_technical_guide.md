...
版本：2026-07-11（对应球面几何、拓扑和质量契约修复后的代码状态）
性质：实现级技术文档。所有公式、常量与索引约定均以当前 Rust 源码核验；模块名即 `rust/earthmesh_mesh/src/` 下的目录名，便于对照阅读。
...
3. **栅格尺寸**（2026-08 起由 Project lowering 推导）：第 2 点的"间距 ≤ h，理想 ≤ h/2"不是建议而是硬约束——h 场在**三角形中心与边中点**处被采样，栅格粗于目标单元时 `level_at` 采出的是混叠后的锯齿边界，Method-C 拿到的是不合法掩膜，最终在 `perim_fill3` 深处报 `transition patch has no solid split edge`。引擎默认 720×360 的经向间距约 55 km，**连 NXP 81 单级所需的 49 km 都不满足**（实测：同一 namelist 仅改栅格，720×360 在 pass 1 失败，2048×1024 通过 pass 1）。

   **注意"理想 ≤ h/2"这句仍然不够**：实测 NXP 81 单级在恰好 `h_min/2`（24.7 km）仍然失败，`h_min/4`（12.35 km）才通过，`h_min/8` 无额外收益。两级之所以在自己的 `h_min/2` 通过，是因为其 `h_min` 更小、绝对间距同样落在 ~12 km——**真正的约束是绝对间距，不是相对 h 的比值**。

   `earthmesh_project` 的 `hfield_raster_size()` 因此按下式推导，显式 `nlon`/`nlat` 优先：

   ```text
   h_min = 2πR / (5 · NXP · 2^L)
   nlat  = ceil(πR / (h_min/4))
   nlon  = 2 · nlat
   ```

   下界 360（不使任何既有配置变粗），上界 8192。

   **闭式**：上式化简即 `nlat = 10·NXP·2^L`，`nlon = 2·nlat`。

   **栅格存在可用窗口，两端都会坏**（2026-08 实测，NXP 21 两级）：

   | 栅格 | 请求锚点 | 覆盖 | 被裁 | 最终单元 | 结果 |
   |---|---|---|---|---|---|
   | 720×360（旧默认） | 38165 | 38165 | 0 | 46516 | 通过 |
   | 840×420（推导值） | 37746 | 37746 | 0 | 46653 | 通过 |
   | 3360×1680（h_min/16） | 10927 | 1728 | **9199** | **8160** | 曾静默交付欠细化网格 |

   过粗 → 选面锯齿 → `perim_fill3` 显式报错；**过细 → 需求碎片化 → 多数锚点缺少完整 rad3 足迹被裁 → 静默欠细化**，后者更危险。因此 `selected_faces_and_coverage_*` 在「被裁 > 覆盖」时改为硬错误并提示可调项——健康算例实测裁剪量恒为 0，而病态算例达 84%。裁掉父层围裙上的锚点仍是合法设计，故判据取"多数丢失"而非"任何裁剪"。

   注意：`h_min/4` 只在窗口内的若干点上验证过，**窗口上界尚未标定**。

   **三个待定项（2026-08，尚未定论，记录在此以便后续决策）**：

   1. **上界取值**。当前 8192，为避免 L≥2 被截断而设，但既未权衡内存、也未确认是否仍在可用窗口内（见上表：过细会触发需求裁剪）：单个场 `nlon·nlat·8` 字节，NXP 81/L=2 已达 160 MB，L=3 达 641 MB，封顶处 1 GB；而 `build_composed_hfield` 逐判据源建场取 min、`constrain_hfield_to_domain` 再跑一遍限制器，峰值是数倍。压回 4096（峰值 268 MB）会让 NXP ≥ 41 且 L=3 被截断到不足——那类配置本应先缩小域。**倾向压回，待定。**
   2. **既有项目的输出会变**。栅格变 → `level_at` 采样变 → 选面变 → 产出网格与旧版不同。这是修复的固有代价（旧网格建立在欠采样场上），但对"网格喂给模式"的用法是重要信息，是否需要在首次以新版运行旧项目时显式提示，**待定**。
   3. **按域裁剪**。基于耗时实测（零收益）本已判定不做；若内存成为约束则重新成立，可省约 227 倍单元数。**待定，取决于第 1 项如何取舍。**

   **栅格代价实测为零**：同一算例 842×421 与 3240×1620 均耗时 42 s——15 倍单元数、相同总时长，梯度限制远不是瓶颈，网格生成才是。因此这里刻意取保守值，而不是去找"恰好够用"的最小栅格；同理，按域裁剪栅格（`HField` 目前无 origin/extent，隐含全球）**不值得做**：数学上安全（留 `(h_base−h_min)/g` 余量即与全球计算逐点相同，NXP 81 两级约 370 km），单元数可省约 227 倍，但省不出时间。
