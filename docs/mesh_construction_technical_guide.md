...
**剩下的 32.3° 对 75.6° 大概率是结构性的**,理由是两个后端造网格的方式不同:
...
**这不等于 HARP-DV 追不上,而是说追上需要改插点方式而不是改后处理**:比如一次插入一整
个模板(而不是一个点),或者在细化过程中交替插点与平滑(Method-C 每嵌套一层就弹一次,
HARP-DV 目前只在最后弹一次)。后者便宜且没试过,是下一个该做的。
...
至此廉价的后处理杠杆全部用尽,32.3° 对 75.6° 的差距**要靠改插点方式才能动**——一次插入
一整个模板而不是一个点,那是重新设计而不是调参。
...
**1 个外边界、0 个洞**,模型校验通过。**报告而不强制**:这条路径上还没有「细化前」的那对数可比,
...
**其余未做的**:那对数目前只报告不强制——这条路径上还没有「细化前」的数可比,凭空定阈值就是编一个
没人测过的数字。HARP-DV 的 `AdaptiveMesh` 仍用自带的 `BTreeSet` 存段列表(它的分裂归纳本身是对的),
换成 `SegmentList` 会牵动回滚路径,单独一刀再做。
...
**还留着一个形状上的隐患**:`contains` 需要定向 → `closed_rings` 给不出定向 → `orient_counter_clockwise`
