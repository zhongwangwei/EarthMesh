...
| **Project HField 栅格自动预算** | `production` | `rust/earthmesh_project/src/lowering/mod.rs` 按 `base_m / 2^max_level` 选择 `nlon/nlat`，隐式上限 `3600×1800`；schema 允许显式覆盖；回归 `implicit_hfield_raster_tracks_finest_requested_level` | NXP=81、max-level=2 自动为 `1800×900`，max-level=3 自动为 `3600×1800`。Case 9 同一 HField 在 `720×360` 触发 pass-2 Valence，而 `3600×1800` 三层均首轮 materialize 成功并在 `438.063 s` 内完成，说明 nlon/nlat 会改变量化后的需求形态和最终拓扑，不能只视为性能参数 |
...
| **Method-C 目标层级闭包** | `production` | `spawn_nest_from_target_levels_internal`（`method_c_spawn_hfield/mod.rs`）；共享 phase-support 遍历不再用 intermediate demand 截断（`:947-950`）；回归 `method_c_hfield_spawn.rs:508`；覆盖校验 `MethodCHfieldDemandCoverage::validate` | 这是共享 Tri/Hex 选择语义，不按产品或 landcover 类别分支。历史 `case9_native_15arcsec_20260729` 的失败来自把原生 landcover 当 topology demand；当前产品路径由自动 `3600×1800` HField 驱动 Method-C，原生 15″ landcover 只用于最终 mask。最新 Case 9 为 264810 Tri cells、`target_above_actual=0`，整网拓扑通过 |
...
| **mask 后孤立分量清理** | `production` | `cleanup_masked_topology_one_based`（`rust/earthmesh_cli/src/masked_topology_cleanup.rs`）← `rust/earthmesh_cli/src/regional_gridfile_writers/landtype.rs`、`rust/earthmesh_cli/src/mask_postproc_ocean/renewal.rs`；`remove_isolated_ocean_one_based` ← `renewal.rs` | 独立 Ocean 最终 LandType mask 使用确定性的 `KeepLargest`，先以无 hard-demand 的结果定义 simulation-ready support，再归一化产品 hard demand，避免孤立水体被重新保留；Land 保留所有非 singleton 分量，LOC coupled partition 不做单侧删除。Case 9 从 92 个 ocean components 降到 1，移除 1450 cells。另有一套 `remove_isolated_refined_cells` 走 refine_planner，语义不同 |
...
| **真实 MERIT-Hydro/CaMa 闭环** | `production`，real-data E2E 已通过 | 耐久入口 `scripts/run_real_hydro_e2e.sh`；测试 `rust/earthmesh_cli/tests/project_hydro_real_e2e.rs:49`；真实 MERIT 跨日期线测试 `project_coast_refinement.rs:410` | 最新真实小窗复验 coarse 1 cell → final 3 cells；Hydro 目标 HField 单独驱动 topology，源 namelist 的 `refine_cal` 被 adapter 关闭，原生 landcover 仅供最终 mask/coupling。coupling 与最终质量均非 fail；日志 `target/real-hydro-target-only.log` |
...
| # | 案例 | 覆盖维度 | 关键设置 |
|---|---|---|---|
| 1 | 全球大气六边形 + 中纬圆形细化 | 全球 / 六边形 / circle / level 2 | **[2026-08-03 已通过]** G-CIRCLE、NXP81、max-level=2、5000 次 refine spring：116129 Hex cells，零自交/非法 polygon/拓扑 fail，`target_above_actual=0`、hard support 全覆盖；质量 `warn`（变分辨率阈值仍 provisional）。证据 `target/matrix-m0-gcircle-current.json` |
| 2 | 全球陆面六边形 + 碎片化阈值场 | 全球 / 六边形 / 碎片化阈值 / level 1–2 | **[2026-08-03 已通过]** G-FRAGMENT、NXP81、max-level=2、500 次 refine spring：78212 Hex cells，零自交/非法 polygon/拓扑 fail，`target_above_actual=0`、hard support 全覆盖；质量 `warn`（变分辨率阈值仍 provisional）。证据 `target/matrix-m0-gfragment-current.json` |
| 3 | 全球 + 跨日期变更线 bbox | 换日线 / bbox / 六边形 | **[2026-08-03 已复验]** NXP=81、`w=170/e=-170/s=-20/n=20`、两层、5000 次 spring：89945 cells，22231 个 level≥2 cells；`lon>170` / `lon<-170` 分别 10129 / 10248，`χ=2`、零边界/拓扑/几何错误、`target_above_actual=0`；SHA256 `cfae78054dffb4e58b6a68dff494fa7360e6e3acf104d405babe0a9dcdb1df5d` |
| 4 | 全球 + 北极圆形细化 | 北极 / circle / 六边形 | **[2026-08-03 已复验]** NXP=81、中心 `(30°,89°)`、半径 600 km、两层、5000 次 spring：69818 cells，3061 个 level≥2 cells，细化纬度 `82.234064°–89.999997°`；南北极均存在、`χ=2`、零边界/拓扑/几何错误、`target_above_actual=0`；SHA256 `89194bc066042e3d4a9d8a1f0cb8bce732a10af49ea70342bf35d72cb187f6e5` |
| 5 | 全球 + 南极 polygon 细化 | 南极 / polygon / 六边形 | **[2026-08-03 明确拒绝]** planar close-ring 路径不支持包围南北极的 SHP polygon；`assemble_polygon_component` 在 lowering 前返回 `polar SHP polygons...`，避免静默错误。回归 `close_source::tests::polar_polygon_is_rejected_before_planar_close_lowering` 已通过。南极极冠使用已验证的 great-circle `specified_circle` 或 `enclosing_cap` |
...
| 7 | 区域六边形 + 闭合流域（含内部孔洞） | 区域 / 流域 / 内部孔洞 / close | **[2026-08-03 已通过]** `scripts/run_basin_hole_regression.sh` 在自动 HField 栅格预算下复验：173 Hex cells、实际 max-level=2、120 个 specified hard-support bins、`target_above_actual=0`、`χ=0=expected`、单连通、2 个边界环、拓扑全绿、质量 `pass`；确定性 hash 为 `0e9f85ea8fde539c57fe6d766d2e38606df089ff0f9f663336387090a7ebb75f`，证据为 `target/basin-hole-regression-1785689497/basin-hole-regression.json`。旧 211-cell/hash 基线已由自动栅格预算有意替代 |
| 8 | 区域海洋三角形 + CoastalOcean 精确海陆 mask + 小分量清理 | 区域 / 三角形 / mask / 孤立分量 | **[2026-08-03 已通过]** 区域 Ocean Tri 最终 mask/FVCOM/OBC 路径由 `binary_mask_restart_area_judge_can_generate_ocean_final_postproc_gridfile` 直接覆盖；最大分量策略由 `largest_policy_drops_smaller_components` 锁定。输出拓扑与边界文件均生成成功。**⚠ 见下方 tri/hex 口径注** |
| 9 | 全球海洋三角形 + landcover mask | 全球 / 三角形 / mask | **[2026-08-03 已通过产品路径]** NXP=81、max-level=3、自动 `3600×1800` HField 在 `438.063 s` 内完成；原生 `86400×43200` landcover 只在最终 Ocean mask 消费，不参与 HField、也未降采样。最终 `264810` Tri cells，92 个 ocean components 清理为最大单一分量，移除 1450 cells；`target_above_actual=0`、相邻层级跳变 ≤1、拓扑全绿，质量仅 provisional angle-deviation `warn`。SHA256 `99bef3cde3b18ee77b01a23a7b196c082bb079e946da40997085afc2935f796c`。旧原生-demand 与 projected-HField 路径保留为历史调查证据，不再代表当前产品流程。**⚠ 见下方 tri/hex 口径注** |
...
| 11 | MeritHydro corridor（窄走廊） | hydro / corridor / 走廊窄于栅格 | **[2026-08-03 已通过真实 E2E]** Hydro adapter 以 Hydro 目标 HField 作为唯一拓扑 demand，不再把源 namelist 的 `refine_cal` landcover 阈值重放成第二个碎片 demand；原生 landcover 仍保留给最终 mask/coupling，未降采样。真实 MERIT/CaMa 小窗 coarse 1 cell → final 3 cells，coupling/质量均非 fail；证据 `target/real-hydro-target-only.log`。 |
...
极区 hash 为
`89194bc066042e3d4a9d8a1f0cb8bce732a10af49ea70342bf35d72cb187f6e5`，
日期变更线 hash 为
`cfae78054dffb4e58b6a68dff494fa7360e6e3acf104d405babe0a9dcdb1df5d`；
...
#### [执行更新 2026-08-02] 当前工作树再次闭合原生 15″ Case 9

最新工作树修复了两个独立的收尾问题：重复掩码时只在已确认的 Valence cycle 上尝试
稳定 parent-M 定向填充；当当前 pass 已无选中面时，跳过只用于诊断的 legalization
checkpoint/preflight，并让生产 spawn 正常返回 `None`。没有把空周界改成合法周界，也没有
放宽 exact emitter 的任何拓扑门。

原生 `86400×43200`、NXP=243、`max_level=3` 的 release 整网复验记录在
`target/case9-empty-checkpoint-guard-1785621946/`：退出码 `0`、耗时 `2078 s`，最终
pass 3 face demand `0/1190318`；质量门 `pass`，`842399` 个 Tri cells，hard-support
欠覆盖、target-above-actual、自交、非法多边形和 topology issue 全部为 `0`。gridfile
SHA256 为 `c5cadcbddc97e46431c8f44c92f33a1abe5c050748a53c569807ac1e9a7f4ff6`。

本次哈希与先前 `842241`-cell 成功路径不同，反映最新选择/支撑语义已改变；因此当前结论是
“最新工作树 E2E 通过”，而不是“与旧产物位级一致”。同一可执行文件的第二次独立运行
记录在 `target/case9-determinism-1785625826/`：退出码 `0`、耗时 `2063 s`，gridfile
SHA256 仍为 `c5cadcbddc97e46431c8f44c92f33a1abe5c050748a53c569807ac1e9a7f4ff6`，
六个 legalization checkpoint SHA256 也逐项一致，当前工作树的确定性已锁定。Beam/跨层
支撑仍保持默认关闭，直至产品矩阵完成。
