...
|---|---:|
| triangle angles below the selected final minimum | 0 |
| triangle angles above the selected final maximum | 0 |
...
Legacy construction uses the stricter internal angle window 40.2--79.8 degrees
and publishes against 40--80 degrees. The DQX geometry contract instead uses
38.2--81.8 internally and publishes against 38--82 degrees; its 40--80 window
is a preference, not a completed spatial-quality guarantee.
...
Published CMRC gridfiles also carry `earthmesh_m_refine_level` and
`earthmesh_w_refine_level`. For a land/ocean carve these arrays are compacted
with the retained cells, so the file itself records the delivered 3→2→1→0
level pattern rather than relying only on the JSON certificate.
...
For `landmesh` and `oceanmesh`, that matrix describes the certified closed
sphere before the land-type subset and is therefore published as
