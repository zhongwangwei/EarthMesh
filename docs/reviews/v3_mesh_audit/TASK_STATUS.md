...
| R4 | MeshQualityReport MVP | Done | dev | 2026-06-22 | 2026-06-22 | [`fix_reports/R4_mesh_quality_report.md`](./fix_reports/R4_mesh_quality_report.md) | 自动 post-run 报告(接 run() 各分支)；hex 视图；球面面积(area_cv warn 根因)；接 QualityConstraintConfig | 已做并验证:新 `earthmesh_quality` crate(geometry+topology metrics + 4 输出 + 阈值,10 测试绿)+ cli `--mesh-quality` 子命令(编译/真实 gridfile 冒烟绿,3D 角度修日期线假阳性)。下一步:自动 post-run + R5 coupling 守恒 |
| R5 | Topology Validator + Repair Hooks | Done | dev | 2026-06-22 | 2026-06-22 | [`fix_reports/R5_topology_validator.md`](./fix_reports/R5_topology_validator.md) | 自动 post-run 接线；cli 路径填 refine_level/邻接以启用 transition/isolated；repair 落地引擎(本期不做) | 已做并验证:`quality::topology`(MeshTopologyValidator 10 validate_* + TopologyIssue + 5 repair hooks + RepairReport)接入 MeshQualityReport+JSON/MD;17 测试绿;cli 集成(真实 gridfile→topology_issues 输出,干净网格 0 issue)。下一步:自动 post-run + R6 |
| R6 | MERIT-Hydro/Hydro-Coast Validation Report MVP | Done | dev | 2026-06-22 | 2026-06-22 | [`fix_reports/R6_hydro_coast.md`](./fix_reports/R6_hydro_coast.md) | cli 接线(MERIT pipeline→HydroCoastInputs)；reader 行为修(跨180°/stride 聚合)非诊断部分；score 实现(R7+) | 已做并验证:`quality::hydro_coast`(HydroCoastValidationReport + validate_bbox/tile_coverage/river_mouth_candidates/build_report + score 占位)覆盖 15 优先项的诊断;26 测试绿。下一步:cli 接线 + R7 |
