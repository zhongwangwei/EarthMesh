...
_MASK_PRIORITY = {"BACKGROUND": 0, "COAST": 10, "R2": 20, "R3": 30}
...
def _merge_properties(
    ...
    *,
    ...
    merged["is_hydro_masked"] = mask_class != "BACKGROUND"
    return merged
...
def merge_cell_masks(
    ...
    *,
    ...
    Priority is R3 > R2 > COAST > BACKGROUND.  Output geometry is always the
    background EarthMesh cell geometry, so the mask is a complete cell table rather
    than a sparse overlay.
    """

    if background_mask_class not in _MASK_PRIORITY or background_mask_class != "BACKGROUND":
        raise ValueError("background_mask_class currently must be BACKGROUND")
    ...
    for base in _features(background_cells):
        ...
        candidates: list[tuple[str, dict[str, object] | None]] = [("BACKGROUND", None)]
        ...
        feature["properties"] = _merge_properties(base, coast=coast, river=river, mask_class=mask_class)
