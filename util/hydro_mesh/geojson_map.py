...
def render_mesh_leaflet_html(
    ...
    *,
    ...
    """Render a Leaflet QA map with embedded background, surface, river, and coast cells."""
    ...
    surface_cells = surface_cells or {"type": "FeatureCollection", "features": []}
    surface_json = json.dumps(surface_cells, sort_keys=True, ensure_ascii=False)
    ...
    surface_count = len(surface_cells.get("features", [])) if isinstance(surface_cells.get("features"), list) else 0
    return f"""<!doctype html>
<html lang=\"en\">
<head>
  <meta charset=\"utf-8\" />
...
<body>
  ...
  <aside class=\"panel\">
    <h1>{title_text}</h1>
    <p>{background_count} background cells, {surface_count} complete LAND/OCEAN cell mask features, {river_count} river-overlap cells, and {coast_count} coastal-overlap EarthMesh cells embedded in this file.</p>
    <div class=\"legend-item\"><span class=\"swatch\" style=\"background: rgba(148,163,184,0.22)\"></span>land/background cells</div>
    <div class=\"legend-item\"><span class=\"swatch\" style=\"background: rgba(34,197,94,0.34)\"></span>LAND surface cells</div>
    <div class=\"legend-item\"><span class=\"swatch\" style=\"background: rgba(59,130,246,0.34)\"></span>OCEAN surface cells</div>
  ...
  <script>
    ...
    const surfaceCells = {surface_json};
    ...
    const surfaceLayer = L.geoJSON(surfaceCells, {{
      style: function(feature) {{
        const p = feature.properties || {{}};
        const cls = p.surface_class || p.mask_class || '';
        const isOcean = cls === 'OCEAN' || cls === 'COAST_OCEAN';
        const isLand = cls === 'LAND' || cls === 'COAST_LAND';
        return {{
          color: isOcean ? '#2563eb' : (isLand ? '#16a34a' : '#64748b'),
          weight: 0.7,
          opacity: 0.38,
          fillColor: isOcean ? '#3b82f6' : (isLand ? '#22c55e' : '#94a3b8'),
          fillOpacity: isOcean ? 0.24 : (isLand ? 0.20 : 0.10)
        }};
      }},
      onEachFeature: function(feature, layer) {{
        layer.bindPopup(popupHtml(feature));
    ...
    L.control.layers({{ 'OpenStreetMap': base }}, {{
      ...
      'complete LAND/OCEAN cell mask': surfaceLayer,
      'coastal-overlap EarthMesh cells': coastLayer,
      'R2/R3 river-overlap cells': riverLayer
    }}, {{ collapsed: false }}).addTo(map);

    const bounds = L.featureGroup([backgroundLayer, surfaceLayer, coastLayer, riverLayer]).getBounds();
    if (bounds.isValid()) {{
...
def mesh_geojson_to_leaflet_html(
    ...
    *,
    ...
    surface = json.loads(Path(surface_geojson).read_text()) if surface_geojson is not None else None
    rendered = render_mesh_leaflet_html(background, rivers, coast_cells=coast, surface_cells=surface, title=title)
    output_path = Path(output_html)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(rendered)
    return rendered
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--surface-geojson", help="Optional complete LAND/OCEAN cell mask GeoJSON for a surface-aware mesh-cell map")
    ...
    elif args.coast_geojson or args.surface_geojson:
        raise ValueError("--coast-geojson and --surface-geojson require --background-geojson")
    else:
