use earthmesh_mesh::refine_iter_b_judge_fortran_indexed;

#[test]
fn iter_b_marks_unrefined_cell_hit_by_two_existing_four_refinements() {
    // Fortran-indexed rows. Cells 4 and 5 are already refined (mrl_new=4).
    ...
    let ref_sjx = refine_iter_b_judge_fortran_indexed(1, 1, &ngrmm, &mrl_new)
...
#[test]
fn iter_b_rejects_zero_neighbor_ids_like_invalid_fortran_connectivity() {
    let ngrmm = vec![
        vec![0, 0, 0],
        vec![0, 0, 0],
        vec![0, 3, 4],
    ...
    let err = refine_iter_b_judge_fortran_indexed(1, 1, &ngrmm, &mrl_new)
