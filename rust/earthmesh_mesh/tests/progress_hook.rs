...
fn spring_loop_reports_progress() {
    ...
    let state = earthmesh_mesh::gridinit_voronoi_state_canonical(6, 60, 1.0, 0.035, 100_000);
...
fn cancellation_aborts_gridinit() {
    ...
    let state = earthmesh_mesh::gridinit_voronoi_state_canonical(6, 5000, 1.0, 0.035, 100_000);
    earthmesh_core::progress::clear();
...
#[test]
fn method_c_nest_spring_reports_at_canonical_nprnt_interval() {
    let reports = Arc::new(Mutex::new(Vec::new()));
    let recorded = reports.clone();
    earthmesh_core::progress::set(move |phase, done, _total| {
        if phase == "method_c-nest-spring" {
            recorded.lock().expect("record progress").push(done);
        }
        true
    });

    let mesh = earthmesh_mesh::MethodCDelaunayMesh::from_icosahedron(6, 0, 1.0, 0.25, 100)
        .expect("base Method-C mesh");
    let region = earthmesh_mesh::MethodCRefinementRegion::Circle {
    ...
    assert!(result.is_ok(), "Method-C nest spring should succeed");
    assert_eq!(
        reports.lock().expect("read progress").as_slice(),
        &[1, 100, 120],
        "Canonical spring_dynamics_nest reports iter 1, iter mod nprnt == 0 with nprnt=100, and final iter"
    );
}
