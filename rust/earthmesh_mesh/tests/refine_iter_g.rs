use earthmesh_mesh::refine_iter_g_judge_fortran_indexed;

#[test]
fn iter_g_marks_unrefined_triangles_in_six_edge_weak_concavity() {
    // Fortran-indexed rows: polygons start after num_center.
    ...
    let ref_sjx =
        refine_iter_g_judge_fortran_indexed(1, 2, &triangles_on_cell, &edge_counts, &mrl_new)
...
fn iter_g_ignores_non_hexagonal_or_non_eighteen_sum_cells() {
    ...
    let ref_sjx =
        refine_iter_g_judge_fortran_indexed(1, 3, &triangles_on_cell, &edge_counts, &mrl_new)
            .expect("calculate iterG weak-concavity marks");
...
fn iter_g_rejects_edge_count_that_exceeds_available_neighbors() {
    ...
    let err = refine_iter_g_judge_fortran_indexed(1, 2, &triangles_on_cell, &edge_counts, &mrl_new)
