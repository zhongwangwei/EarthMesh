use earthmesh_mesh::{
    ...
    find_frac_index_canonical, set_dists_on_edge_global_one_based, DistanceLayerSpacing,
...
#[test]
fn find_frac_index_matches_canonical_ascending_grid() {
    let hit = find_frac_index_canonical(&[-180.0, -90.0, 0.0, 90.0, 180.0], -45.0)
...
#[test]
fn find_frac_index_matches_canonical_descending_grid() {
    let hit = find_frac_index_canonical(&[90.0, 45.0, 0.0, -45.0, -90.0], 22.5)
...
#[test]
fn find_frac_index_clamps_boundary_fraction_like_canonical() {
    let left = find_frac_index_canonical(&[0.0, 10.0, 20.0], 0.0).expect("left boundary");
    assert_eq!(left.index, 1);
    approx_eq(left.frac, 0.0, 1.0e-15);

    let right = find_frac_index_canonical(&[0.0, 10.0, 20.0], 20.0).expect("right boundary");
...
fn find_frac_index_rejects_points_outside_grid() {
    assert!(find_frac_index_canonical(&[0.0, 10.0, 20.0], -1.0).is_none());
    assert!(find_frac_index_canonical(&[0.0, 10.0, 20.0], 21.0).is_none());
}

#[test]
fn distance_layers_match_canonical_linear_formula() {
...
#[test]
fn distance_layers_match_canonical_nonlinear_formulas_at_bounds() {
...
#[test]
fn dists_on_edge_layers_marks_refined_and_first_halo_edges_like_canonical() {
    ...
    let updated = dists_on_edge_layers_one_based(
        1,
        1,
        0,
        1,
...
fn dists_on_edge_layers_rejects_short_dist_layer_array() {
    ...
    assert!(dists_on_edge_layers_one_based(
        1,
        1,
        0,
        1,
...
#[test]
fn cellwidth_layers_marks_refined_and_first_halo_cells_like_canonical() {
...
fn cellwidth_layers_rejects_short_dist_layer_array() {
    ...
    assert!(cellwidth_layers_one_based(
        1,
        1,
        0,
        1,
...
fn set_dists_on_edge_global_wrapper_matches_manual_layer_sequence() {
    ...
    let output = set_dists_on_edge_global_one_based(SetDistsOnEdgeGlobalInput {
    ...
    let expected_edges = dists_on_edge_layers_one_based(
        1,
        1,
        0,
        1,
    ...
    let expected_cellwidth = cellwidth_layers_one_based(
        1,
        1,
        0,
        1,
