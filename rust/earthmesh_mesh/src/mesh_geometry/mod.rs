...
pub fn lookup_method_c_midpoint(
    ...
    midpoint_by_edge
        .get(&method_c_edge_key(im1, im2))
        .copied()
        .ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                format!("W face {owner_iw} canonicals edge [{im1}, {im2}] without a midpoint"),
            )
        })
}
...
pub fn lookup_method_c_thirds(
    ...
    let points_from_low_to_high = thirds_by_edge
        ...
        .ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                format!("W face {owner_iw} canonicals edge [{im1}, {im2}] without thirds"),
            )
        })?;
    if im1 <= im2 {
