//! The mesh every backend produces and every writer consumes.
//!
//! Three reciprocal tables:
//!
//! - M: Delaunay vertices, and the centres of the Voronoi cells they become.
//! - U: Delaunay edges.
//! - W: Delaunay triangle faces, and the corners of those Voronoi cells.
//!
//! Plus, per row, which refinement generation produced it and which row it
//! descends from.
//!
//! # Why this is not Method-C's, though it lived in Method-C's module
//!
//! Measured rather than assumed. Of the fields that looked private to the
//! nesting, `mrlm`, `mrlm_orig` and `ngr` are read outside it exactly once --
//! by `voronoi_grid`, filling `ItabW`, because the *gridfile* carries them.
//! `w_lineage` and `m_lineage` are read outside it not at all, and a lineage is
//! what any refinement produces: red-green carries one too, as
//! `RedGreenOutcome::cell_renumbering`.
//!
//! One field really is Method-C's -- `boundary_rows`, the mrow transition rows
//! -- and it is still here, declared beside the rest. Taking it out needs a
//! Method-C-side type to put it in, which is the next step and not this one.
//!
//! # Why the split needs more than moving files
//!
//! Rust has no inherent impl of a foreign type, and Method-C is written as 97
//! methods across 33 `impl` blocks on this struct. Until they hang off a type
//! Method-C owns, no file move puts them in another crate. That, not the module
//! count, is what has kept the nesting in here.
