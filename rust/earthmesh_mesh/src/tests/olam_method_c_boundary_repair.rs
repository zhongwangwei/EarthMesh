...
fn olam_method_c_matches_reduced_fortran_nxp7_two_circle_summary() {
    let mesh = OlamDelaunayMesh::from_icosahedron(7, 0, 1.0, 0.25, 100).expect("base OLAM mesh");
    let regions = [
        OlamRefinementRegion::Circle {
            center: LonLatDegrees::new(115.0, 25.0),
...
fn olam_method_c_rejects_reduced_fortran_nxp6_two_circle_too_close_boundary() {
    ...
    let error = mesh.spawn_nest_as_atmosmesh(&regions, 2).expect_err(
        "reduced Fortran probe rejects this two-level circle as too close to the parent boundary",
    );
    let message = error.to_string();
    assert!(
            message.contains("crosses")
