...
fn olam_region_start_prefers_contained_global_pentagon() {
    ...
    let start = mesh
        .olam_refinement_start_point_with_neighbors(&region, radius, &method_c_m_neighbors, false)
...
fn olam_thirdm_walks_straight_opposite_edges_and_marks_reciprocal_done_like_fortran() {
    let mesh =
        OlamDelaunayMesh::from_icosahedron(6, 0, 1.0, 0.25, 100).expect("base mesh should build");
