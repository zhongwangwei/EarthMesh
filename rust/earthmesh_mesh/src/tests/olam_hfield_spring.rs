...
/// mrow tests).
fn refined_test_mesh() -> OlamDelaunayMesh {
    ...
    mesh.spawn_nest_with_max_mrows(&[region], 1, OlamDelaunayMesh::METHOD_C_MAX_MROWS_SURFACE)
        .expect("Method-C nest")
}

fn max_ngr(mesh: &OlamDelaunayMesh) -> usize {
    (2..=mesh.nmd)
...
/// Legacy-equivalent per-edge targets: `dist00 / 2^(mrlu - 1)`, optionally
        ...
        .map(|iu| {
            ...
            if fold_mrow_multiplier {
                ...
                if iw1 > 1 && iw1 <= mesh.nwd && iw2 > 1 && iw2 <= mesh.nwd {
                    target *= olam_nest_mrow_distance_multiplier(
...
#[test]
fn hfield_scratch_matches_legacy_masks_floor_and_unshaped_targets_bitwise() {
    ...
    let movable = crate::olam_nest_spring::olam_nest_movable_m_points(&refined, ngr, false)
        .expect("movable mask");
    assert!(
    ...
    let legacy = OlamNestSpringScratch::new(&refined, &topology, &movable, dist00, true)
        .expect("legacy scratch");
    let targets = level_derived_targets(&refined, edge_count, dist00, false);
    let hmode = OlamNestSpringScratch::with_edge_target_lengths(
        &refined, &topology, &movable, &targets, true,
    )
    .expect("h-field scratch");

    assert_eq!(legacy.moveu, hmode.moveu);
    assert_eq!(legacy.compu, hmode.compu);
    assert_eq!(
        legacy.min_area_squared.to_bits(),
        hmode.min_area_squared.to_bits(),
        "area floor must match bitwise (power-of-two scaling is exact)"
    );
    assert_eq!(legacy.radius, hmode.radius);
    ...
    for iu in 2..edge_count {
        ...
        if legacy.target_mrow_multiplier[iu] == 1.0 {
            unshaped += 1;
            assert_eq!(
                legacy.target_level_base[iu].to_bits(),
...
#[test]
fn hfield_spring_with_level_derived_targets_tracks_legacy_and_is_deterministic() {
    let refined = refined_test_mesh();
    let ngr = max_ngr(&refined);
    let niter = 40usize;
    ...
    let legacy = refined
        ...
        .expect("legacy nest spring");

    let radius = crate::olam_spring::active_mesh_radius(&refined).expect("mesh radius");
    let dist00 = olam_fortran_global_dist00(1.0, radius, 6);
    let topology = icosahedron_spring_topology_fortran(
        refined.nmd,
        &refined.u_edges,
        &refined.m_neighbors,
    ...
    assert_eq!(hmode.nmd, legacy.nmd);
    assert_eq!(hmode.nud, legacy.nud);
    assert_eq!(hmode.nwd, legacy.nwd);

    let movable = crate::olam_nest_spring::olam_nest_movable_m_points(&refined, ngr, false)
        .expect("movable mask");
    let mut max_diff = 0.0_f64;
    for im in 2..=refined.nmd {
        let a = legacy.m_points[im];
    ...
    assert!(
        ...
        "h-field spring drifted from legacy: {max_diff} (radius {radius})"
    );
}
...
fn edge_targets_sample_at_midpoints_and_move_only_transition_points() {
    ...
    let targets = olam_edge_target_lengths_from_field(&refined, |_lon, _lat| dist00)
