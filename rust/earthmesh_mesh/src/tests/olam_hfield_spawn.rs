...
fn face_centroid_lonlat(mesh: &OlamDelaunayMesh, iw: usize) -> LonLatDegrees {
...
fn single_level_field_spawn_matches_region_spawn_footprint() {
    ...
    let from_field = mesh
        .spawn_nest_from_target_levels(field, 1, OlamDelaunayMesh::METHOD_C_MAX_MROWS_SURFACE)
    ...
    let apron_m = (OlamDelaunayMesh::METHOD_C_MAX_MROWS_SURFACE as f64 + 2.0) * 1_500_000.0;
...
fn two_level_field_spawn_nests_and_deeper_passes_stop_cleanly() {
    ...
    // spawnable by the legacy path on this fixture. If THIS expect fires, the
    ...
    let from_region = mesh
        .spawn_nest_with_max_mrows(&regions, 2, OlamDelaunayMesh::METHOD_C_MAX_MROWS_SURFACE)
        .expect("region-mode two-level spawn must be feasible on this fixture");

    let field = two_ring_levels(inner_m, outer_m);
    let two = mesh
        .spawn_nest_from_target_levels(&field, 2, OlamDelaunayMesh::METHOD_C_MAX_MROWS_SURFACE)
    ...
    let child_apron_m = (OlamDelaunayMesh::METHOD_C_MAX_MROWS_SURFACE as f64 + 2.0) * 800_000.0;
    for iw in 2..=two.nwd {
    ...
    let five = mesh
        .spawn_nest_from_target_levels(&field, 5, OlamDelaunayMesh::METHOD_C_MAX_MROWS_SURFACE)
...
fn empty_field_is_identity_and_zero_level_rejected_paths() {
    ...
    let out = mesh
        .spawn_nest_from_target_levels(silent, 3, OlamDelaunayMesh::METHOD_C_MAX_MROWS_SURFACE)
