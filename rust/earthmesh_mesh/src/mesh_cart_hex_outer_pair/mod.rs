...
pub(crate) fn order_method_c_outer_w_pair_for_fill_rad3(
    ...
    if let Some(ordered) =
        order_method_c_outer_w_pair_candidate(w_faces, pair, outer_candidates, imx)?
    {
        return Ok(ordered);
    }
    if let Some(ordered) =
        order_method_c_outer_w_pair_candidate(w_faces, [pair[1], pair[0]], outer_candidates, imx)?
    {
        return Ok(ordered);
    }
    ...
    if std::env::var_os("EARTHMESH_METHOD_C_DEBUG").is_some() {
        eprintln!(
            "EARTHMESH_METHOD_C_DEBUG: outer W pair {pair:?} around M point {imx} \
...
fn order_method_c_outer_w_pair_candidate(
    w_faces: &[IcosahedronWFace],
    pair: [usize; 2],
    outer_candidates: [usize; 6],
    imx: usize,
    ...
    require_method_c_id("Method-C cart_hex outer W pair", pair[0], nwd)?;
    require_method_c_id("Method-C cart_hex outer W pair", pair[1], nwd)?;
    ...
    for iwy in w_faces[pair[0]].iw {
        ...
        require_method_c_id("Method-C cart_hex iwx W neighbor", iwy, nwd)?;
    ...
    for iwy in outer_candidates {
        require_method_c_id("Method-C cart_hex outer W candidate", iwy, nwd)?;
        if iwy != pair[0] && w_faces[iwy].im.contains(&im2) {
            let im3 = face_following_vertex(w_faces[iwy], im2, iwy)?;
            if im3 != im1 {
                return Ok(Some([pair[0], iwy]));
