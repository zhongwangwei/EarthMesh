...
use crate::{
    netcdf_to_io_error, rad_to_deg, require_len, required_dimension_len, required_values_f64_any,
    required_values_i32_matrix, LonLatPoint,
};

/// Edge-canonical payload read by `MOD_file_preprocess.F90:data_read`.
#[derive(Debug, Clone, PartialEq)]
pub struct MpasEdgeIndex {
    pub cells_on_edge_canonical: Vec<[i32; 2]>,
    pub edge_points: Vec<LonLatPoint>,
}

/// Read the MPAS edge-canonical fields consumed by
/// `MOD_file_preprocess.F90:data_read`.
///
/// The returned payload preserves the Canonical placeholder row at index `0`,
/// shifts `cellsOnEdge` by `+1` after reading, converts edge coordinates from
/// radians to degrees, and applies the compatibility single-step `lon > 180 => lon -=
/// 360` normalization.
pub fn read_mpas_edge_index_netcdf(input: impl AsRef<Path>) -> io::Result<MpasEdgeIndex> {
    ...
    let lon_edge = required_values_f64_any(&file, "lonEdge")?;
    let lat_edge = required_values_f64_any(&file, "latEdge")?;
    require_len("lonEdge", lon_edge.len(), n_edges)?;
    require_len("latEdge", lat_edge.len(), n_edges)?;

    let mut cells_on_edge_canonical = Vec::with_capacity(n_edges + 1);
    cells_on_edge_canonical.push([1, 1]);
    for edge in 0..n_edges {
        let base = edge * 2;
        cells_on_edge_canonical.push([cells[base] + 1, cells[base + 1] + 1]);
    }
