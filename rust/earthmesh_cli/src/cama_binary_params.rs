...
fn parse_cama_params_text(text: &str) -> io::Result<CamaBinaryGridSpec> {
    ...
    if values.iter().any(|value| !value.is_finite()) {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "CaMa params numeric records must be finite",
        ));
    }
...
fn parse_cama_dimension(value: f64, name: &str) -> io::Result<usize> {
    if value <= 0.0 || value.fract() != 0.0 || value > i32::MAX as f64 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!("CaMa {name} must be a positive integer in the safe nextxy/isize range"),
        ));
    }
    Ok(value as usize)
}

fn validate_cama_extent(
    axis: &str,
    low: f64,
    high: f64,
    count: usize,
    ...
    if high <= low {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!("CaMa {axis} bounds must be strictly increasing"),
        ));
    }
    let expected = count as f64 * grid_size_deg;
    let actual = high - low;
    let tolerance = expected.abs().max(grid_size_deg.abs()).max(1.0) * 1.0e-6;
    if !expected.is_finite() || (actual - expected).abs() > tolerance {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "CaMa {axis} extent {actual} is inconsistent with {count} cells at {grid_size_deg} degrees"
            ),
        ));
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn cama_params_reject_non_finite_fractional_and_unsafe_dimensions() {
        for text in [
            "NaN\n3\n1\n0.5\n100\n102\n10\n11.5\n",
            "4.5\n3\n1\n0.5\n100\n102\n10\n11.5\n",
            "2147483648\n3\n1\n0.5\n100\n1073741924\n10\n11.5\n",
        ] {
            assert!(parse_cama_params_text(text).is_err(), "accepted {text:?}");
        }
    }

    #[test]
    fn cama_params_reject_inconsistent_or_non_finite_geometry() {
        for text in [
            "4\n3\n1\n0.5\n100\n103\n10\n11.5\n",
            "4\n3\n1\n0.5\n100\n102\n10\n12\n",
            "4\n3\n1\ninf\n100\n102\n10\n11.5\n",
