#[test]
fn earth_patchtypes_follow_land_ratio_and_canonical_pixel_mapping() {
    let contain = earthmesh_cli::contain_io::ContainMesh {
        // Canonical row 1 placeholder; rows 2 and 3 are active domain cells.
    ...
    let result = earthmesh_cli::mask_postproc_patchtypes::build_earth_patchtypes_one_based(
        &contain, 0.4, 10, 20, 2, 2,
    )
...
#[test]
fn earth_patchtypes_allows_empty_pixels_when_no_domain_cells_canonical_pixels() {
    let contain = earthmesh_cli::contain_io::ContainMesh {
...
fn earth_patchtypes_reject_bad_schema_and_out_of_range_pixels() {
    ...
    let err = earthmesh_cli::mask_postproc_patchtypes::build_earth_patchtypes_one_based(
        &short_pixels,
        0.5,
        10,
        20,
    ...
    let bad_coordinate = earthmesh_cli::contain_io::ContainMesh {
        ustr_id: vec![vec![0, 0], vec![1, 1]],
