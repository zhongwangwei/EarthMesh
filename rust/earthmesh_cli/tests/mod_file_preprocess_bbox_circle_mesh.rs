...
use earthmesh_cli::{BBoxMesh, BBoxPoint, CircleMesh, LonLatPoint};

#[test]
fn bbox_mesh_read_write_preserves_fortran_schema_without_refine_field() {
    ...
    earthmesh_cli::write_bbox_mesh_netcdf(&output, &mesh).expect("write bbox mesh");
    ...
    let roundtrip = earthmesh_cli::read_bbox_mesh_netcdf(&output).expect("read bbox mesh");
...
#[test]
fn circle_mesh_read_write_preserves_fortran_schema_without_refine_field() {
    ...
    earthmesh_cli::write_circle_mesh_netcdf(&output, &mesh).expect("write circle mesh");
    ...
    let roundtrip = earthmesh_cli::read_circle_mesh_netcdf(&output).expect("read circle mesh");
    assert_eq!(roundtrip, mesh);
