...
fn bbox_mask_round_trips() {
    ...
    let mask = earthmesh_cli::bbox_mask_io::BBoxMask {
        refine_degree: 0,
    ...
    earthmesh_cli::bbox_mask_io::write_bbox_mask_netcdf(&path, &mask).expect("write bbox mask");
    let read = earthmesh_cli::bbox_mask_io::read_bbox_mask_netcdf(&path).expect("read bbox mask");
...
fn circle_mask_round_trips() {
    ...
    let mask = earthmesh_cli::circle_close_mask_io::CircleMask {
        refine_degree: 0,
    ...
    earthmesh_cli::circle_close_mask_io::write_circle_mask_netcdf(&path, &mask)
        .expect("write circle mask");
    let read = earthmesh_cli::circle_close_mask_io::read_circle_mask_netcdf(&path)
        .expect("read circle mask");
    assert_eq!(read.points.len(), 1);
