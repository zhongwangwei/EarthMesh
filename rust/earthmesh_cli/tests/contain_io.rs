#[test]
fn contain_reader_writer_round_trip_legacy_schema() {
    let root =
    ...
    let report = earthmesh_cli::write_contain_netcdf(&output, &contain).expect("write contain");
    ...
    let read_back = earthmesh_cli::read_contain_netcdf(&output).expect("read contain");
...
fn contain_writer_rejects_ragged_rows_and_mask_length_mismatch() {
    ...
    let ragged = earthmesh_cli::ContainMesh {
        ustr_id: vec![vec![1, 1], vec![2]],
        ustr_ii: vec![vec![1, 2]],
        is_in_area_ustr: vec![1, 1],
    };
    let err = earthmesh_cli::write_contain_netcdf(&output, &ragged).expect_err("ragged rejected");
    assert!(err
        .to_string()
        .contains("ustr_id rows must have uniform width"));

    let bad_mask = earthmesh_cli::ContainMesh {
        ustr_id: vec![vec![1, 1]],
        ustr_ii: vec![vec![1, 2]],
        is_in_area_ustr: vec![1, 1],
    };
    let err = earthmesh_cli::write_contain_netcdf(&output, &bad_mask).expect_err("mask mismatch");
