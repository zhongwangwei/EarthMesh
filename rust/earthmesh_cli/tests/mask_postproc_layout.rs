#[test]
fn mask_postproc_layout_uses_tri_centers_and_hex_swaps_centers_vertices_like_canonical() {
    ...
    let tri = earthmesh_cli::mask_postproc_layout::mask_postproc_layout_from_unstructured_mesh(
        &mesh, "tri",
    )
    .expect("tri layout");
    ...
    let hex = earthmesh_cli::mask_postproc_layout::mask_postproc_layout_from_unstructured_mesh(
        &mesh, "hex",
    )
    .expect("hex layout");
...
fn mask_postproc_layout_rejects_unsupported_mode_grid_and_negative_connectivity() {
    let mesh = earthmesh_cli::unstructured_mesh_support::UnstructuredMesh {
        ...
        w_points: vec![earthmesh_cli::coordinate_types::LonLatPoint { lon: 0.0, lat: 0.0 }],
        m_to_w: vec![[1, -1, 1]],
        w_to_m: vec![vec![1]],
        n_w_to_m: vec![1],
    };
