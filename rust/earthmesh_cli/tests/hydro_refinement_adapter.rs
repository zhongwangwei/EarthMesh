...
fn hydro_target_levels_execute_the_real_method_c_pipeline() {
    ...
    assert!(adapter_text.contains("NL%refine_backend = 'method_c'"));
    assert!(
    ...
    assert!(
        earthmesh_core::EarthmeshConfig::from_mkgrd_namelist(
            &fs::read_to_string(root.join("quality_pass_2/adapter.nml")).unwrap()
        )
        .unwrap()
        .defer_model_exports
    );
