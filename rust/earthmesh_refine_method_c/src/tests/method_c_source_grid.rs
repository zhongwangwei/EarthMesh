...
fn method_c_source_grid_style_multilevel_corridor_table_outputs_closed_mesh() {
    ...
    let refined = mesh
        ...
        .expect("source-grid-style atmosphere Method-C corridor table nest");
    assert_eq!(
            (refined.nmd, refined.nud, refined.nwd),
            (84_099, 252_289, 168_193),
            "source-grid-style atmosphere Method-C corridor table output should match the Canonical Method-C HDF5 M/U/W table sizes"
        );
    ...
    assert_eq!(
            ...
            "source-grid-style atmosphere Method-C corridor table output should match the Canonical Method-C per-grid W-face counts"
        );
    ...
    assert_eq!(
            ...
            "source-grid-style atmosphere Method-C corridor table output should match the Canonical Method-C atmosphere mrow envelope"
        );
    refined
        .validate_topology()
        .expect("source-grid-style Method-C table topology");
}
...
fn method_c_source_grid_style_multilevel_corridor_outputs_closed_mesh() {
    ...
    let (refined, spring_passes) = mesh
        ...
        .expect("source-grid-style atmosphere Method-C corridor nest");
    assert_eq!(
        spring_passes, 3,
        "Canonical MAKEGRID runs one atmosphere spring pass after each active Method-C nest"
    );
    assert_eq!(
            (refined.nmd, refined.nud, refined.nwd),
            (84_099, 252_289, 168_193),
            "source-grid-style atmosphere Method-C corridor output should match the Canonical Method-C HDF5 M/U/W table sizes"
        );
    let mut ngr_counts = BTreeMap::<usize, usize>::new();
    for face in refined.w_faces.iter().skip(2) {
        *ngr_counts.entry(face.ngr).or_insert(0) += 1;
    ...
    assert_eq!(
            ngr_counts,
            BTreeMap::from([(1, 76_426), (2, 11_468), (3, 15_114), (4, 65_184)]),
            "source-grid-style atmosphere Method-C corridor output should match the Canonical Method-C per-grid W-face counts"
        );
    let mut mrows = refined
        .w_faces
        .iter()
    ...
    assert_eq!(
            (mrows.first().copied(), mrows.last().copied(), mrows.len()),
            (Some(-13), Some(13), 50_069),
            "source-grid-style atmosphere Method-C corridor output should match the Canonical Method-C atmosphere mrow envelope"
        );

    refined
        .validate_topology()
        .expect("source-grid-style Method-C topology");
    ...
    assert_eq!(
        grid_numbers,
        BTreeSet::from([2, 3, 4]),
        "Canonical Method-C spawns one grid number per source-grid corridor refinement level"
    );
    ...
    for im in 2..=mesh.nmd {
        assert!(
            ...
            "old M point {im} exceeds Method-C-supported valence after source-grid-style Method-C nesting"
        );
    }
    ...
    .expect("source-grid-style Method-C Voronoi handoff");
