                ...
                Err(error) if Self::is_repairable_method_c_transition_error(&error) => {
                    ...
                    if repaired.is_none() {
                        repaired = if let Some(im) = valence_m {
                            // `im` is a parent M point because `emit_method_c_tables`
                            // makes it one before raising.
                            //
                            // The derivation that raises the valence error walks
                            // the mesh being built, so its own id names a point
                            // there, and emission renumbers -- `imnew` advances one
                            // per parent M point and one more for every subdivided
                            // U edge. This guard used to be `im <= self.nmd`, a
                            // bound check standing in for a validity check: above
                            // `nmd` it skipped the repair that should have run, and
                            // below it filled around an unrelated point, 379 times
