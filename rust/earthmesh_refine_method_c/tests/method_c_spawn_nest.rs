...
fn spawn_nest_cartesian_xy_keeps_method_c_points_unprojected_like_canonical_mdomain_ge_two() {
    ...
    let refined = mesh
        .spawn_nest_cartesian_xy_with_max_mrows(&[region], 1, MethodCMesh::MAX_MROWS_ATMOS)
        ...
        refined
            ...
            .any(|(point, metadata)| {
                metadata.ngr > 1
                    ...
                    && (magnitude(*point) - METHOD_C_CANONICAL_EARTH_RADIUS_METERS).abs() > 1.0
            }),
        "Canonical spawn_nest only projects coordinates back to Earth radius when mdomain < 2"
    );
}
...
fn spawn_nest_cart_hex_preserves_canonical_periodic_prognostic_maps() {
    ...
    assert!(
        refined
            ...
            .count()
            >= original_m_copies,
        "Method-C spawn must preserve Canonical cart_hex M periodic copy mapping"
    );
    assert!(
        refined
            .u_prognostic
            ...
            .count()
            >= original_u_copies,
        "Method-C spawn must preserve Canonical cart_hex U periodic copy mapping"
    );
    assert!(
        refined
            .w_prognostic
            ...
            .count()
            >= original_w_copies,
        "Method-C spawn must preserve Canonical cart_hex W periodic copy mapping"
    );
}
...
fn spawn_nest_cartesian_xy_spring_keeps_movable_points_unprojected_like_canonical_mdomain_ge_two() {
    ...
    assert!(
        movable_child_points.iter().any(|&point_id| {
            (magnitude(refined.m_points[point_id]) - METHOD_C_CANONICAL_EARTH_RADIUS_METERS).abs() > 1.0
        }),
        "Canonical spring_dynamics_nest only projects moved M points back to Earth radius when mdomain < 2"
    );
}
...
fn spawn_nest_cartesian_xy_spring_uses_canonical_deltax_target_distance_for_mdomain_ge_two() {
    ...
    assert!(
        ...
        "Canonical spring_dynamics_nest uses NL%deltax, not spherical NXP spacing, for mdomain >= 2"
    );
}
...
fn spawn_nest_uses_one_imbeg_for_disconnected_same_level_regions_like_canonical_method_c() {
    ...
    let refined = mesh
        ...
        .expect("Canonical Method-C starts from one IMBEG for one spawned grid");
    ...
    assert_eq!(
        ...
        "one Canonical spawn_nest pass should create one spawned grid, not sibling grids"
    );
    refined
        .validate_topology()
...
fn spawn_nest_uses_one_imbeg_for_disconnected_mixed_level_regions_like_canonical_method_c() {
    ...
    let refined = mesh
        ...
        .expect("Canonical Method-C starts each pass from one IMBEG");
    let spawned_grid_numbers = refined
        .w_faces
        .iter()
        .skip(2)
    ...
    assert_eq!(
        ...
        "two Canonical spawn_nest passes should create one spawned grid per pass"
    );
    refined
        .validate_topology()
...
fn spawn_nest_uses_current_levels_only_when_max_level_exceeds_input() {
    ...
    let refined = mesh
        .spawn_nest(&[region], 5)
        .expect("Canonical Method-C should not spawn empty passes");
    let spawned_grid_numbers = refined
        .w_faces
        .iter()
        .skip(2)
...
fn spawn_nest_continues_grid_numbers_on_already_nested_mesh() {
    ...
    assert!(
        ...
        "Canonical grid numbers are globally unique across spawned grids; no ngr greater than {max_existing_ngr} was created"
    );
}
...
fn spawn_nest_two_level_china_regions_keep_method_c_valence_limit() {
    ...
    let refined = mesh
        .spawn_nest(&regions, 5)
        .expect("Canonical Method-C uses one spawned grid per pass");
    ...
    for im in 2..=mesh.nmd {
        assert!(
            ...
            "old M point {im} exceeds Method-C-supported valence after Method-C nesting"
        );
    }
}
...
fn spawn_nest_rejects_two_level_polygon_without_explicit_parent_halo_like_canonical() {
    ...
    let error = mesh
        .spawn_nest(&[region], 5)
        .expect_err("Canonical Method-C does not synthesize a parent halo for future grids");
    assert!(
...
fn debug_single_south_america_method_c_region() {
    ...
    let error = mesh
        .spawn_nest(&[region], 1)
        .expect_err("Canonical Method-C rejects this coarse South America region");
...
fn spawn_nest_rejects_overlapping_china_regions_with_method_c_perimeter_error() {
    ...
    let error = mesh
        .spawn_nest(&regions, 5)
        .expect_err("Canonical Method-C rejects overlapping local-rebuild-style nests");
...
fn spawn_nest_with_spring_uses_one_imbeg_for_disconnected_regions() {
    ...
    let (refined, _) = mesh
        ...
        .expect("Canonical Method-C uses one spawned grid before spring relaxation");
...
fn spawn_nest_uses_method_c_surface_transition_row_width() {
    ...
    assert!(
        max_abs_mrow >= 4,
        "Method-C surface spawn_nest should keep a multi-row transition band; got max |mrow|={max_abs_mrow}"
    );
}
...
fn spawn_nest_explicit_max_mrows_controls_transition_width() {
    ...
    assert!(wide_max_abs_mrow <= MethodCMesh::MAX_MROWS_ATMOS);
    assert!(narrow_max_abs_mrow <= MethodCMesh::MAX_MROWS_SURFACE);
}
...
fn spawn_nest_default_width_matches_surface_max_mrows() {
    ...
    assert_eq!(
        ...
        "spawn_nest default width should match MAX_MROWS_SURFACE"
    );
}
...
fn spawn_nest_tracks_canonical_m_point_grid_metadata() {
    ...
    assert_eq!(
        metadata.len(),
        refined.nmd + 1,
        "M metadata should keep Method-C's one-based table layout"
    );
    assert!(
        metadata
            .iter()
        ...
        "spawn_nest should mark refined M points with Canonical itab_md mrlm/mrlm_orig/ngr"
    );
    assert!(
        metadata
            .iter()
    ...
    assert_eq!(
        max_mrlm_orig, 2,
        "Canonical perim_fill3 sets transition M mrlm_orig to parent mrlo + 1, not max_level or grid number"
    );
    assert!(
        refined
            .w_faces
    ...
    assert!(
        ...
        "Canonical perim_fill3 marks transition W faces with the child grid number while preserving parent mrlw"
    );
}
...
fn spawn_nest_marks_perimeter_m_points_with_current_grid_number() {
    ...
    assert!(
        mismatched.is_empty(),
        "Canonical perim_mrow marks every M point adjacent to W faces on current ngr; mismatched M ids: {mismatched:?}"
    );
}
...
fn method_c_nest_spring_moves_transition_points_without_changing_topology() {
    ...
    let smoothed = refined
        ...
        .expect("Method-C nest spring adjustment");
    ...
    assert!(
        ...
        "Method-C nest spring should move perturbed transition M point"
    );
}

#[test]
fn spawn_nest_rejects_invalid_refinement_levels() {
    let mesh = MethodCMesh::from_icosahedron(3, 0, 1.0, 0.25, 100).expect("base Method-C mesh");
    let region = RefinementRegion::Circle {
        center: LonLatDegrees::new(0.0, 0.0),
