...
pub fn read_method_c_domain_region(config: &EarthmeshConfig) -> io::Result<Option<GridRegion>> {
...
pub fn read_method_c_bbox_domain_regions(
    source: &Path,
...
pub fn read_method_c_circle_domain_regions(
    source: &Path,
    regions: &mut Vec<GridRegion>,
) -> io::Result<()> {
    let mask = match source_extension(source).as_deref() {
...
pub fn read_method_c_close_domain_regions(
    source: &Path,
