...
pub fn rectilinear_cells_contain_cells(
...
pub fn axis_aligned_rectangle_from_ring(
    coordinates: &[(f64, f64)],
...
pub fn rectilinear_ring_cells(coordinates: &[(f64, f64)]) -> Option<Vec<(f64, f64, f64, f64)>> {
...
pub fn is_rectilinear_ring(coordinates: &[(f64, f64)]) -> bool {
    coordinates
        .iter()
...
pub fn rectilinear_union_cells(
...
pub fn rectilinear_union_boundary(rectangles: &[(f64, f64, f64, f64)]) -> Option<Vec<(f64, f64)>> {
    let mut xs = rectangles
        .iter()
        .flat_map(|(min_lon, _, max_lon, _)| [*min_lon, *max_lon])
        .collect::<Vec<_>>();
...
pub fn trace_rectilinear_boundary(edges: Vec<((f64, f64), (f64, f64))>) -> Option<Vec<(f64, f64)>> {
    trace_polygon_boundary(edges).map(remove_collinear_boundary_vertices)
}

pub fn trace_polygon_boundary(mut edges: Vec<((f64, f64), (f64, f64))>) -> Option<Vec<(f64, f64)>> {
    if edges.is_empty() {
        return None;
    }
    let start = edges
...
pub fn remove_collinear_boundary_vertices(mut boundary: Vec<(f64, f64)>) -> Vec<(f64, f64)> {
    if boundary.len() <= 3 {
        return boundary;
    }
    loop {
