...
pub fn convex_polygon_intersection_area(left: &[(f64, f64)], right: &[(f64, f64)]) -> Option<f64> {
...
pub fn convex_hull_points<I>(points: I) -> Option<Vec<(f64, f64)>>
where
...
pub fn is_convex_ring(coordinates: &[(f64, f64)]) -> bool {
...
pub fn ring_contains_ring(outer: &[(f64, f64)], inner: &[(f64, f64)]) -> bool {
    outer.len() >= 3
        && inner.len() >= 3
        && inner
            .iter()
...
pub fn point_on_ring_boundary(point: (f64, f64), coordinates: &[(f64, f64)]) -> bool {
...
pub fn polygon_ring_edges(coordinates: &[(f64, f64)]) -> Option<Vec<((f64, f64), (f64, f64))>> {
    if coordinates.len() < 3 {
        return None;
    }
...
pub fn edge_midpoint_is_strictly_inside_ring(
...
pub fn point_in_ring(point: (f64, f64), coordinates: &[(f64, f64)]) -> bool {
    let (x, y) = point;
