...
pub fn cross_2d(origin: (f64, f64), left: (f64, f64), right: (f64, f64)) -> f64 {
...
pub fn point_on_segment(point: (f64, f64), start: (f64, f64), end: (f64, f64)) -> bool {
...
pub fn segment_parameter(point: (f64, f64), start: (f64, f64), end: (f64, f64)) -> f64 {
...
pub fn point_in_rectangle(point: (f64, f64), rect: (f64, f64, f64, f64)) -> bool {
    point.0 > rect.0 && point.0 < rect.2 && point.1 > rect.1 && point.1 < rect.3
}

pub fn sort_dedup_f64(values: &mut Vec<f64>) {
    values.sort_by(|left, right| left.partial_cmp(right).unwrap_or(std::cmp::Ordering::Equal));
    values.dedup_by(|left, right| (*left - *right).abs() <= 1.0e-12);
}

pub fn points_equal(left: (f64, f64), right: (f64, f64)) -> bool {
    (left.0 - right.0).abs() <= 1.0e-12 && (left.1 - right.1).abs() <= 1.0e-12
}

pub fn are_collinear(a: (f64, f64), b: (f64, f64), c: (f64, f64)) -> bool {
    ((b.0 - a.0) * (c.1 - b.1) - (b.1 - a.1) * (c.0 - b.0)).abs() <= 1.0e-12
}

pub fn ring_bbox(coordinates: &[(f64, f64)]) -> (f64, f64, f64, f64) {
...
pub fn rectangle_ring(rect: (f64, f64, f64, f64)) -> Vec<(f64, f64)> {
    vec![
        (rect.0, rect.1),
        (rect.2, rect.1),
        (rect.2, rect.3),
