...
pub fn validate_output_destination(output: &Path) -> io::Result<()> {
...
pub fn validate_output_path(input: &Path, output: &Path) -> io::Result<()> {
...
pub fn atomic_write(output: &Path, write: impl FnOnce(&Path) -> io::Result<()>) -> io::Result<()> {
    let parent = output
        .parent()
        .filter(|p| !p.as_os_str().is_empty())
        .unwrap_or_else(|| Path::new("."));
