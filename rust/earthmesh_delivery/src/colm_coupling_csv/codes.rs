...
pub fn surface_class_code(value: &str) -> i8 {
...
pub fn colm_land_fraction(row: &ColmCouplingCsvRow) -> f64 {
...
pub const COLM_F64_FILL_VALUE: f64 = 9.969_209_968_386_869e36;

pub fn finite_or_fill(value: f64) -> f64 {
    if value.is_finite() {
        value
    } else {
        COLM_F64_FILL_VALUE
...
pub fn fraction_or_fill(value: f64) -> f64 {
    if value.is_finite() {
        value.clamp(0.0, 1.0)
    } else {
        COLM_F64_FILL_VALUE
...
pub fn fraction_or_zero(value: f64) -> f64 {
    if value.is_finite() {
        value.clamp(0.0, 1.0)
    } else {
        0.0
...
pub fn river_class_code(value: &str) -> i8 {
    match value.trim().to_ascii_uppercase().as_str() {
        "R1" => 1,
        "R2" => 2,
        "R3" => 3,
...
pub fn coast_class_code(value: &str) -> i8 {
    match value.trim().to_ascii_uppercase().as_str() {
        "COAST" => 1,
        "COAST_LAND" => 2,
        "COAST_OCEAN" => 3,
