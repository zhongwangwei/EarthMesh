...
pub fn usize_from_i32_connectivity(value: i32, name: &str) -> io::Result<usize> {
...
pub fn usize_from_i32_nonnegative(value: i32, name: &str) -> io::Result<usize> {
    usize::try_from(value).map_err(|_| {
        io::Error::new(
...
pub fn usize_from_i32_positive(value: i32, name: &str) -> io::Result<usize> {
...
pub fn lookup_f64(values: &[f64], index: usize, name: &str) -> io::Result<f64> {
...
pub fn m_to_w_as_usize_rows(rows: &[[i32; 3]]) -> io::Result<Vec<Vec<usize>>> {
    rows.iter()
        .map(|row| {
            row.iter()
...
pub fn i32_rows_as_usize(rows: &[Vec<i32>], name: &str) -> io::Result<Vec<Vec<usize>>> {
    rows.iter()
        .map(|row| {
            row.iter()
...
pub fn i32_counts_as_usize(values: &[i32], name: &str) -> io::Result<Vec<usize>> {
    values
        .iter()
        .map(|&value| usize_from_i32_connectivity(value, name))
        .collect()
...
pub fn validate_mask_postproc_layout(layout: &MaskPostprocLayout) -> io::Result<()> {
...
pub fn lonlat_pairs_from_points(points: &[LonLatPoint]) -> Vec<[f64; 2]> {
    points.iter().map(|point| [point.lon, point.lat]).collect()
}

pub fn lonlat_points_from_pairs(
    name: &str,
    values: &[[f64; 2]],
...
pub fn rows_to_triangle_connectivity(
    name: &str,
    rows: &[Vec<usize>],
    expected_final_id: usize,
) -> io::Result<Vec<[i32; 3]>> {
...
pub fn usize_rows_to_i32(name: &str, rows: &[Vec<usize>]) -> io::Result<Vec<Vec<i32>>> {
    rows.iter()
        .map(|row| usize_values_to_i32(name, row))
        .collect()
}

pub fn usize_to_i32(name: &str, value: usize) -> io::Result<i32> {
...
pub fn lonlat_degrees_from_points(points: &[LonLatPoint]) -> Vec<LonLatDegrees> {
    points
        .iter()
...
pub fn split_cartesian_components(points: &[CartesianPoint]) -> (Vec<f64>, Vec<f64>, Vec<f64>) {
...
pub fn scale_cartesian_points_by_earth_radius(points: &mut [CartesianPoint]) {
    for point in points {
...
pub fn usize_values_to_i32(name: &str, values: &[usize]) -> io::Result<Vec<i32>> {
    values
        .iter()
