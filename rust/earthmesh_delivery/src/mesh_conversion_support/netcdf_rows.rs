...
pub fn f64_matrix_width(name: &str, rows: &[Vec<f64>]) -> io::Result<usize> {
...
pub fn matrix_width<T>(name: &str, rows: &[Vec<T>]) -> io::Result<usize> {
    let width = rows.first().map(Vec::len).unwrap_or(0);
    if rows.iter().any(|row| row.len() != width) {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
...
pub fn flatten_i32_rows(rows: &[Vec<i32>]) -> Vec<i32> {
...
pub fn one_to_n_i32(n: usize, name: &str) -> io::Result<Vec<i32>> {
...
pub fn rows_from_flat_i32(values: &[i32], width: usize) -> Vec<Vec<i32>> {
    if width == 0 {
        return Vec::new();
    }
